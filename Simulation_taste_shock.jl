...
panel_S_c = zeros(num_S, num_periods)
...
panel_C_c = zeros(num_C, num_periods)
panel_C_age = zeros(Int, num_C, num_periods)

# Divorced
panel_div_1_a = zeros(Int, num_div, num_periods)
panel_div_2_a = zeros(Int, num_div, num_periods)
panel_div_1_n = zeros(Int, num_div, num_periods)
panel_div_2_n = zeros(Int, num_div, num_periods)
panel_div_1_d = zeros(Int, num_div, num_periods)
panel_div_2_d = zeros(Int, num_div, num_periods)
panel_div_1_c = zeros(num_div, num_periods)
panel_div_2_c = zeros(num_div, num_periods)
panel_div_1_age = zeros(Int, num_div, num_periods)
panel_div_2_age = zeros(Int, num_div, num_periods)
panel_div_1_ind = zeros(Int, num_div, num_periods)
panel_div_2_ind = zeros(Int, num_div, num_periods)
...
shock_C_div = zeros(Int, num_C, num_periods)
# persistent productivity
...
# Divorced
# survival rate
shock_div_1_ρ = rand(Categorical([parameters.ρ, 1-parameters.ρ]), (num_div, num_periods))
shock_div_2_ρ = rand(Categorical([parameters.ρ, 1-parameters.ρ]), (num_div, num_periods))
# persistent productivity
shock_div_1_z = zeros(Int, num_div, num_periods)
shock_div_2_z = zeros(Int, num_div, num_periods)
# transitory productivity
shock_div_1_η = zeros(Int, num_div, num_periods)
shock_div_2_η = zeros(Int, num_div, num_periods)

# Auxiliary matrices
# Singles
σ_S_temp = reshape(σ_S,:,parameters.a_size,parameters.z_size,parameters.η_size)
temp_S = zeros(parameters.a_size+1,parameters.n_size)

# Divorced
σ_div_temp = reshape(σ_div,:,parameters.a_size,parameters.z_size,parameters.η_size)
temp_div = zeros(parameters.a_size+1,parameters.n_size)

# Couples
σ_C_temp = reshape(σ_C,:,parameters.a_size,parameters.z_size,parameters.z_size,parameters.η_size,parameters.η_size)
temp_C = zeros(parameters.a_size+1,parameters.n_size,parameters.n_size)
...
@showprogress 1 "Computing..." for period_i in 1:(num_periods-1)
    ...
    for hh_i in 1:num_S
        if period_i == 1 || shock_S_ρ[hh_i,period_i] == 2
            ...
            # Compute choices
            action_i = rand(Categorical(σ_S_temp[:,a_i,z_i,η_i]))
            a_p_i = CartesianIndices(temp_S)[action_i][1]
            n_i = CartesianIndices(temp_S)[action_i][2]

            if a_p_i == 1 # Default
                panel_S_d[hh_i,period_i] = 2
                panel_S_a[hh_i,period_i+1] = parameters.a_ind_zero
                panel_S_n[hh_i,period_i] = n_i
                panel_S_c[hh_i,period_i] = (parameters.z_grid[z_i]*parameters._η_grid[η_i]*(parameters.n_grid[n_i]^(parameters.θ)))*(1.0-parameters.ϕ)
            else
                panel_S_d[hh_i,period_i] = 1
                panel_S_a[hh_i,period_i+1] = a_p_i - 1
                panel_S_n[hh_i,period_i] = n_i
                panel_S_c[hh_i,period_i] = parameters.z_grid[z_i]*parameters.η_grid[η_i]*(parameters.n_grid[n_i]^(parameters.θ))+parameters.a_grid[a_i]-q_S[a_p_i-1,z_i]*parameters.a_grid[a_p_i-1]
            end
            ...
            if a_p_i == 1 # Default
                ...
                panel_S_a[hh_i,period_i+1] = parameters.a_ind_zero
                panel_S_n[hh_i,period_i] = n_i
                panel_S_c[hh_i,period_i] = (parameters.z_grid[z_i]*parameters.η_grid[η_i]*(parameters.n_grid[n_i]^(parameters.θ)))*(1.0-parameters.ϕ)
            else
                panel_S_d[hh_i,period_i] = 1
                panel_S_a[hh_i,period_i+1] = a_p_i - 1
                panel_S_n[hh_i,period_i] = n_i
    ...
    # Couples
    for hh_i in 1:num_C
        if period_i == 1 || shock_C_ρ[hh_i,period_i] == 2 || shock_C_div[hh_i,period_i] == 1

            if period_i == 1 || shock_C_ρ[hh_i,period_i] == 2
                shock_C_div[hh_i,period_i] == 2
            end

            if shock_C_div[hh_i,period_i] == 1
                div_1_ind = findfirst(panel_div_1_ind[:,period_i] .== 0)
                div_2_ind = findfirst(panel_div_2_ind[:,period_i] .== 0)

                panel_div_1_ind[div_1_ind,period_i] = 1
                panel_div_2_ind[div_2_ind,period_i] = 1
                ...
                shock_div_1_z[div_1_ind,period_i] = z_1_i
                shock_div_2_z[div_2_ind,period_i] = z_2_i

                a_p = parameters.a_grid[panel_C_a[hh_i,period_i]]
                ...
                panel_div_1_a[div_1_ind,period_i] = a_p_i_half_1
                panel_div_2_a[div_2_ind,period_i] = a_p_i_half_2

                panel_div_1_age[div_1_ind,period_i] = panel_C_age[hh_i,period_i-1] + 1
                panel_div_2_age[div_2_ind,period_i] = panel_C_age[hh_i,period_i-1] + 1
            end

            # Initiate exogenous states for newborns
            shock_C_div[hh_i,period_i+1] = rand(Categorical([parameters.ψ, 1-parameters.ψ]))
            ...
            # Compute choices
            action_i = rand(Categorical(σ_C_temp[:,a_i,z_1_i,z_2_i,η_1_i,η_2_i]))
            a_p_i = CartesianIndices(temp_C)[action_i][1]
            n_1_i = CartesianIndices(temp_C)[action_i][2]
            n_2_i = CartesianIndices(temp_C)[action_i][3]

            if a_p_i == 1 # Default
                panel_C_d[hh_i,period_i] = 2
                panel_C_a[hh_i,period_i+1] = parameters.a_ind_zero
                panel_C_1_n[hh_i,period_i] = n_1_i
                panel_C_2_n[hh_i,period_i] = n_2_i
                panel_C_c[hh_i,period_i] = (parameters.z_grid[z_1_i]*parameters.η_grid[η_1_i]*(parameters.n_grid[n_1_i]^parameters.θ)+parameters.z_grid[z_2_i]*parameters.η_grid[η_2_i]*(parameters.n_grid[n_2_i]^parameters.θ))*(1.0-parameters.ϕ)
            else
                panel_C_d[hh_i,period_i] = 1
                panel_C_a[hh_i,period_i+1] = a_p_i - 1
                panel_C_1_n[hh_i,period_i] = n_1_i
                panel_C_2_n[hh_i,period_i] = n_2_i
                panel_C_c[hh_i,period_i] = parameters.z_grid[z_1_i]*parameters.η_grid[η_1_i]*(parameters.n_grid[n_1_i]^parameters.θ)+parameters.z_grid[z_2_i]*parameters.η_grid[η_2_i]*(parameters.n_grid[n_2_i]^parameters.θ)+parameters.a_grid[a_i]-q_C[a_p_i-1,z_1_i,z_2_i]*parameters.a_grid[a_p_i-1]
            end
    ...
    for hh_i in 1:num_div
        ...
        if (panel_div_1_ind[hh_i,period_i] == 1)
            ...
            if shock_div_1_ρ[hh_i,period_i+1] == 1
                panel_div_1_ind[hh_i,period_i+1] = 2
            end
            # extract exogenous states
            # persistent productivity
            z_1_i = shock_div_1_z[hh_i,period_i]
            # z_2_i = shock_div_2_z[hh_i,period_i]

            # Transitory productivity
            η_1_i = rand(Categorical(parameters.Γ_η))
            shock_div_1_η[hh_i,period_i] = η_1_i

            a_i = panel_div_1_a[hh_i, period_i]

            # Compute choices
            action_i = rand(Categorical(σ_div_temp[:,a_i,z_1_i,η_1_i]))
            a_p_i = CartesianIndices(temp_div)[action_i][1]
            n_1_i = CartesianIndices(temp_div)[action_i][2]

            if a_p_i == 1 # Default
                panel_div_1_d[hh_i,period_i] = 2
                panel_div_1_a[hh_i,period_i+1] = parameters.a_ind_zero
                panel_div_1_n[hh_i,period_i] = n_1_i
                panel_div_1_c[hh_i,period_i] = (parameters.z_grid[z_1_i]*parameters.η_grid[η_1_i]*(parameters.n_grid[n_1_i]^parameters.θ))*(1.0-parameters.ϕ)
            else
                panel_div_1_d[hh_i,period_i] = 1
                panel_div_1_a[hh_i,period_i+1] = a_p_i - 1
                panel_div_1_n[hh_i,period_i] = n_1_i
                panel_div_1_c[hh_i,period_i] = parameters.z_grid[z_1_i]*parameters.η_grid[η_1_i]*(parameters.n_grid[n_1_i]^parameters.θ)+parameters.a_grid[a_i]-q_S[a_p_i-1,z_1_i]*parameters.a_grid[a_p_i-1] - parameters.κ
            end
        end

        if (panel_div_2_ind[hh_i,period_i] == 1)
            # extract exogenous states
            if shock_div_2_ρ[hh_i,period_i+1] == 1
                panel_div_2_ind[hh_i,period_i+1] = 2
            end
            ...
            # Transitory productivity
            η_2_i = rand(Categorical(parameters.Γ_η))
            shock_div_2_η[hh_i,period_i] = η_2_i

            a_i = panel_div_2_a[hh_i, period_i]

            # Compute choices
            action_i = rand(Categorical(σ_div_temp[:,a_i,z_2_i,η_2_i]))
            a_p_i = CartesianIndices(temp_div)[action_i][1]
            n_2_i = CartesianIndices(temp_div)[action_i][2]

            if a_p_i == 1 # Default
                panel_div_2_d[hh_i,period_i] = 2
                panel_div_2_a[hh_i,period_i+1] = parameters.a_ind_zero
                panel_div_2_n[hh_i,period_i] = n_2_i
                panel_div_2_c[hh_i,period_i] = (parameters.z_grid[z_2_i]*parameters.η_grid[η_2_i]*(parameters.n_grid[n_2_i]^parameters.θ))*(1.0-parameters.ϕ)
            else
                panel_div_2_d[hh_i,period_i] = 1
                panel_div_2_a[hh_i,period_i+1] = a_p_i - 1
                panel_div_2_n[hh_i,period_i] = n_2_i
                panel_div_2_c[hh_i,period_i] = parameters.z_grid[z_2_i]*parameters.η_grid[η_2_i]*(parameters.n_grid[n_2_i]^parameters.θ)+parameters.a_grid[a_i]-q_S[a_p_i-1,z_2_i]*parameters.a_grid[a_p_i-1] - parameters.κ
            end
        end
        ...
        if (panel_div_1_ind[hh_i,period_i] == 2)
            ...
            # extract exogenous states
            # persistent productivity
            z_1_i = rand(Categorical(parameters.Γ_z[shock_div_1_z[hh_i,period_i-1],:]))
            shock_div_1_z[hh_i,period_i] = z_1_i

            # Transitory productivity
            η_1_i = rand(Categorical(parameters.Γ_η))
            shock_div_1_η[hh_i,period_i] = η_1_i

            panel_div_1_age[hh_i,period_i] = panel_div_1_age[hh_i,period_i-1] + 1

            a_i = panel_div_1_a[hh_i, period_i]

            # Compute choices
            action_i = rand(Categorical(σ_S_temp[:,a_i,z_1_i,η_1_i]))
            a_p_i = CartesianIndices(temp_div)[action_i][1]
            n_1_i = CartesianIndices(temp_div)[action_i][2]
            ...
            if a_p_i == 1 # Default
                ...
                panel_div_1_a[hh_i,period_i+1] = a_p_i - 1
                panel_div_1_n[hh_i,period_i] = n_1_i
                panel_div_1_c[hh_i,period_i] = parameters.z_grid[z_1_i]*parameters.η_grid[η_1_i]*(parameters.n_grid[n_1_i]^parameters.θ)+parameters.a_grid[a_i]-q_S[a_p_i-1,z_1_i]*parameters.a_grid[a_p_i-1]
            end
        end
        ...
        if (panel_div_2_ind[hh_i,period_i] == 2)
            ...
            # extract exogenous states
            # persistent productivity
            z_2_i = rand(Categorical(parameters.Γ_z[shock_div_2_z[hh_i,period_i-1],:]))
            shock_div_2_z[hh_i,period_i] = z_2_i

            # Transitory productivity
            η_2_i = rand(Categorical(parameters.Γ_η))
            shock_div_2_η[hh_i,period_i] = η_2_i

            panel_div_2_age[hh_i,period_i] = panel_div_2_age[hh_i,period_i-1] + 1

            a_i = panel_div_2_a[hh_i, period_i]

            # Compute choices
            action_i = rand(Categorical(σ_S_temp[:,a_i,z_2_i,η_2_i]))
            a_p_i = CartesianIndices(temp_div)[action_i][1]
            n_2_i = CartesianIndices(temp_div)[action_i][2]
            ...
            if a_p_i == 1 # Default
                ...
                panel_div_2_a[hh_i,period_i+1] = a_p_i - 1
                panel_div_2_n[hh_i,period_i] = n_2_i
                panel_div_2_c[hh_i,period_i] = parameters.z_grid[z_2_i]*parameters.η_grid[η_2_i]*(parameters.n_grid[n_2_i]^parameters.θ)+parameters.a_grid[a_i]-q_S[a_p_i-1,z_2_i]*parameters.a_grid[a_p_i-1]
            end
        end
    end
end
...
panel_S_c = panel_S_c[:,burn_in+1:end-1]
...
# Divorced
panel_div_1_a = panel_div_1_a[:,burn_in+1:end-1]
panel_div_1_d = panel_div_1_d[:,burn_in+1:end-1]
panel_div_1_n = panel_div_1_n[:,burn_in+1:end-1]
panel_div_1_c = panel_div_1_c[:,burn_in+1:end-1]
panel_div_1_age = panel_div_1_age[:,burn_in+1:end-1]
panel_div_1_ind = panel_div_1_ind[:,burn_in+1:end-1]
shock_div_1_ρ = shock_div_1_ρ[:,burn_in+1:end-1]
shock_div_1_z = shock_div_1_z[:,burn_in+1:end-1]
shock_div_1_η = shock_div_1_η[:,burn_in+1:end-1]

panel_div_2_a = panel_div_2_a[:,burn_in+1:end-1]
panel_div_2_d = panel_div_2_d[:,burn_in+1:end-1]
panel_div_2_n = panel_div_2_n[:,burn_in+1:end-1]
panel_div_2_c = panel_div_2_c[:,burn_in+1:end-1]
panel_div_2_age = panel_div_2_age[:,burn_in+1:end-1]
panel_div_2_ind = panel_div_2_ind[:,burn_in+1:end-1]
shock_div_2_ρ = shock_div_2_ρ[:,burn_in+1:end-1]
shock_div_2_z = shock_div_2_z[:,burn_in+1:end-1]
shock_div_2_η = shock_div_2_η[:,burn_in+1:end-1]
...
panel_C_c = panel_C_c[:,burn_in+1:end-1]
panel_C_age = panel_C_age[:,burn_in+1:end-1]
shock_C_ρ = shock_C_ρ[:,burn_in+1:end-1]
shock_C_div = shock_C_div[:,burn_in+1:end-1]
...
#======================#
# Singles
fraction_S_default_sim_all_periods = zeros(period_all)
for i in 1:period_all
    fraction_S_default_sim_all_periods[i] = sum(panel_S_d[:,i] .== 2)/num_S*100
end
fraction_S_default_sim_all_periods_ave = sum(fraction_S_default_sim_all_periods)/period_all
...
# Divorced
fraction_div_default_sim_all_periods = zeros(period_all)
for i in 1:period_all
    fraction_div_default_sim_all_periods[i] = (sum(panel_div_1_d[panel_div_1_ind[:,i] .!= 0,i] .== 2) + sum(panel_div_2_d[panel_div_2_ind[:,i] .!= 0,i] .== 2))/(sum(panel_div_1_ind[:,i] .!= 0) + sum(panel_div_2_ind[:,i] .!= 0))*100
end
fraction_div_default_sim_all_periods_ave = sum(fraction_div_default_sim_all_periods)/period_all
...
# Couples
fraction_C_default_sim_all_periods = zeros(period_all)
for i in 1:period_all
    fraction_C_default_sim_all_periods[i] = sum(panel_C_d[:,i] .== 2)/num_C*100
end
fraction_C_default_sim_all_periods_ave = sum(fraction_C_default_sim_all_periods)/period_all
...
# Among Divorced
fraction_div_loan_users_sim_all_periods = zeros(period_all)
for i in 1:period_all
    fraction_div_loan_users_sim_all_periods[i] = (sum(panel_div_1_a[panel_div_1_ind[:,i] .!= 0,i] .< parameters.a_ind_zero) + sum(panel_div_2_a[panel_div_2_ind[:,i] .!= 0,i] .< parameters.a_ind_zero))/(sum(panel_div_1_ind[:,i] .!= 0) + sum(panel_div_2_ind[:,i] .!= 0))*100
end
fraction_div_loan_users_sim_all_periods_ave = sum(fraction_div_loan_users_sim_all_periods)/period_all
...
# Couples
fraction_C_loan_users_sim_all_periods = zeros(period_all)
for i in 1:period_all
    fraction_C_loan_users_sim_all_periods[i] = sum(panel_C_a[:,i] .< parameters.a_ind_zero)/num_C*100
end
fraction_C_loan_users_sim_all_periods_ave = sum(fraction_C_loan_users_sim_all_periods)/period_all
...
# Singles
debt_S_sim_all_periods = 0.0
income_S_sim_all_periods = 0.0

for i in 1:period_all
    for hh_i in 1:num_S
        if (panel_S_a[hh_i,i] .< parameters.a_ind_zero)
            debt_S_sim_all_periods += abs(parameters.a_grid[panel_S_a[hh_i,i]])
            income_S_sim_all_periods += parameters.z_grid[shock_S_z[hh_i,i]]*parameters.η_grid[shock_S_η[hh_i,i]]*(parameters.n_grid[panel_S_n[hh_i,i]]^(parameters.θ))
        end
    end
end

debt_to_inc_S_sim_all_periods = (debt_S_sim_all_periods/income_S_sim_all_periods)*100

# Among Divorced
debt_div_sim_all_periods = 0.0
income_div_sim_all_periods = 0.0

for i in 1:period_all
    for hh_i in 1:num_div
        if (panel_div_1_a[hh_i,i] .< parameters.a_ind_zero) && (panel_div_1_ind[hh_i,i] .!= 0)
            debt_div_sim_all_periods += abs(parameters.a_grid[panel_div_1_a[hh_i,i]])
            income_div_sim_all_periods += parameters.z_grid[shock_div_1_z[hh_i,i]]*parameters.η_grid[shock_div_1_η[hh_i,i]]*(parameters.n_grid[panel_div_1_n[hh_i,i]]^(parameters.θ))
        end

        if (panel_div_2_a[hh_i,i] .< parameters.a_ind_zero) && (panel_div_2_ind[hh_i,i] .!= 0)
            debt_div_sim_all_periods += abs(parameters.a_grid[panel_div_2_a[hh_i,i]])
            income_div_sim_all_periods += parameters.z_grid[shock_div_2_z[hh_i,i]]*parameters.η_grid[shock_div_2_η[hh_i,i]]*(parameters.n_grid[panel_div_2_n[hh_i,i]]^(parameters.θ))
        end
    end
end

debt_to_inc_div_sim_all_periods = (debt_div_sim_all_periods/income_div_sim_all_periods)*100
...
# Couples
debt_C_sim_all_periods = 0.0
income_C_sim_all_periods = 0.0

for i in 1:period_all
    for hh_i in 1:num_C
        if (panel_C_a[hh_i,i] .< parameters.a_ind_zero)
            debt_C_sim_all_periods += abs(parameters.a_grid[panel_C_a[hh_i,i]])
            income_C_sim_all_periods += parameters.z_grid[shock_C_z_1[hh_i,i]]*parameters.η_grid[shock_C_η_1[hh_i,i]]*(parameters.n_grid[panel_C_1_n[hh_i,i]]^(parameters.θ)) + parameters.z_grid[shock_C_z_2[hh_i,i]]*parameters.η_grid[shock_C_η_2[hh_i,i]]*(parameters.n_grid[panel_C_2_n[hh_i,i]]^(parameters.θ))
        end
    end
end

debt_to_inc_C_sim_all_periods = (debt_C_sim_all_periods/income_C_sim_all_periods)*100
...
# Singles
ave_bank_rate_S_sim_all_periods = zeros(period_all-1)
for i in 2:period_all
    ave_bank_price_S_sim_num = 0.0
    ave_bank_price_S_sim_den = 0.0

    for hh_i in 1:num_S
        a_p_i = panel_S_a[hh_i,i]

        if (a_p_i < parameters.a_ind_zero) && (shock_S_ρ[hh_i,i] == 1)
            ave_bank_price_S_sim_num += q_S[a_p_i,shock_S_z[hh_i,i-1]]
            ave_bank_price_S_sim_den += 1.0
        end
    end

    ave_bank_price_S_sim = ave_bank_price_S_sim_num/ave_bank_price_S_sim_den
    ave_bank_rate_S_sim = (1.0/ave_bank_price_S_sim) - 1.0

    ave_bank_rate_S_sim_all_periods[i-1] = ave_bank_rate_S_sim
end
ave_bank_rate_S_sim_all_periods_ave = sum(ave_bank_rate_S_sim_all_periods)/(period_all-1)

# Among Divorced
ave_bank_rate_div_sim_all_periods = zeros(period_all-1)
for i in 2:period_all
    ...
    for hh_i in 1:num_div
        ...
        if (panel_div_1_a[hh_i,i] .< parameters.a_ind_zero) && (panel_div_1_ind[hh_i,i-1] .!= 0) && (panel_div_1_ind[hh_i,i] .== 2)
            ave_bank_price_div_sim_num += q_S[panel_div_1_a[hh_i,i],shock_div_1_z[hh_i,i-1]]
            ave_bank_price_div_sim_den += 1.0
        end

        if (panel_div_2_a[hh_i,i] .< parameters.a_ind_zero) && (panel_div_2_ind[hh_i,i-1] .!= 0) && (panel_div_2_ind[hh_i,i] .== 2)
            ave_bank_price_div_sim_num += q_S[panel_div_2_a[hh_i,i],shock_div_2_z[hh_i,i-1]]
            ave_bank_price_div_sim_den += 1.0
        end
    end

    ave_bank_price_div_sim = ave_bank_price_div_sim_num/ave_bank_price_div_sim_den
    ave_bank_rate_div_sim = (1.0/ave_bank_price_div_sim) - 1.0

    ave_bank_rate_div_sim_all_periods[i-1] = ave_bank_rate_div_sim
end

ave_bank_rate_div_sim_all_periods_ave = sum(ave_bank_rate_div_sim_all_periods)/(period_all-1)
...
# Couples
ave_bank_rate_C_sim_all_periods = zeros(period_all-1)
for i in 2:period_all
    ave_bank_price_C_sim_num = 0.0
    ave_bank_price_C_sim_den = 0.0

    for hh_i in 1:num_C
        a_p_i = panel_C_a[hh_i,i]

        if a_p_i < parameters.a_ind_zero
            ave_bank_price_C_sim_num += q_C[a_p_i,shock_C_z_1[hh_i,i-1],shock_C_z_2[hh_i,i-1]]
            ave_bank_price_C_sim_den += 1.0
        end
    end

    ave_bank_price_C_sim = ave_bank_price_C_sim_num/ave_bank_price_C_sim_den
    ave_bank_rate_C_sim = (1.0/ave_bank_price_C_sim) - 1.0

    ave_bank_rate_C_sim_all_periods[i-1] = ave_bank_rate_C_sim
end
ave_bank_rate_C_sim_all_periods_ave = sum(ave_bank_rate_C_sim_all_periods)/(period_all-1)
...
# Singles
consumption_S_sim_all_periods_num = 0.0
consumption_S_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_S
        consumption_S_sim_all_periods_num += panel_S_c[hh_i,i]
        consumption_S_sim_all_periods_den += 1.0
    end
end

consumption_S_sim_all_periods_ave = consumption_S_sim_all_periods_num / consumption_S_sim_all_periods_den

# Among divorced
consumption_div_sim_all_periods_num = 0.0
consumption_div_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_div
        if (panel_div_1_ind[hh_i,i] .!= 0)
            consumption_div_sim_all_periods_num += panel_div_1_c[hh_i,i]
            consumption_div_sim_all_periods_den += 1.0
        end

        if (panel_div_2_ind[hh_i,i] .!= 0)
            consumption_div_sim_all_periods_num += panel_div_2_c[hh_i,i]
            consumption_div_sim_all_periods_den += 1.0
        end
    end
end

consumption_div_sim_all_periods_ave = consumption_div_sim_all_periods_num / consumption_div_sim_all_periods_den
...
# Couples
consumption_C_sim_all_periods_num = 0.0
consumption_C_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_C
        consumption_C_sim_all_periods_num += panel_C_c[hh_i,i]
        consumption_C_sim_all_periods_den += 1.0
    end
end

consumption_C_sim_all_periods_ave = consumption_C_sim_all_periods_num / (2*consumption_C_sim_all_periods_den)

#===================#
# 6) Average Labor  #
#===================#
# Singles
labor_S_sim_all_periods_num = 0.0
labor_S_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_S
        labor_S_sim_all_periods_num += parameters.n_grid[panel_S_n[hh_i,i]]
        labor_S_sim_all_periods_den += 1.0
    end
end

labor_S_sim_all_periods_ave = labor_S_sim_all_periods_num / labor_S_sim_all_periods_den

# Among Divorced
labor_div_sim_all_periods_num = 0.0
labor_div_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_div
        if (panel_div_1_ind[hh_i,i] .!= 0)
            labor_div_sim_all_periods_num += parameters.n_grid[panel_div_1_n[hh_i,i]]
            labor_div_sim_all_periods_den += 1.0
        end

        if (panel_div_2_ind[hh_i,i] .!= 0)
            labor_div_sim_all_periods_num += parameters.n_grid[panel_div_2_n[hh_i,i]]
            labor_div_sim_all_periods_den += 1.0
        end
    end
end

labor_div_sim_all_periods_ave = labor_div_sim_all_periods_num / labor_div_sim_all_periods_den
...
# Couples
labor_C_sim_all_periods_num = 0.0
labor_C_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_C
        labor_C_sim_all_periods_num += parameters.n_grid[panel_C_1_n[hh_i,i]] + parameters.n_grid[panel_C_2_n[hh_i,i]]
        labor_C_sim_all_periods_den += 2.0
    end
end

labor_C_sim_all_periods_ave = labor_C_sim_all_periods_num / labor_C_sim_all_periods_den
...
# Singles
assets_S_sim_all_periods_num = 0.0
assets_S_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_S
        assets_S_sim_all_periods_num += parameters.a_grid[panel_S_a[hh_i,i]]
        assets_S_sim_all_periods_den += 1.0
    end
end

assets_S_sim_all_periods_ave = assets_S_sim_all_periods_num / assets_S_sim_all_periods_den

# Among Divorced
assets_div_sim_all_periods_num = 0.0
assets_div_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_div
        if (panel_div_1_ind[hh_i,i] .!= 0)
            assets_div_sim_all_periods_num += parameters.a_grid[panel_div_1_a[hh_i,i]]
            assets_div_sim_all_periods_den += 1.0
        end

        if (panel_div_2_ind[hh_i,i] .!= 0)
            assets_div_sim_all_periods_num += parameters.a_grid[panel_div_2_a[hh_i,i]]
            assets_div_sim_all_periods_den += 1.0
        end
    end
end

assets_div_sim_all_periods_ave = assets_div_sim_all_periods_num / assets_div_sim_all_periods_den
...
# Couples
assets_C_sim_all_periods_num = 0.0
assets_C_sim_all_periods_den = 0.0

for i in 1:period_all
    for hh_i in 1:num_C
        assets_C_sim_all_periods_num += parameters.a_grid[panel_C_a[hh_i,i]]
        assets_C_sim_all_periods_den += 1.0
    end
end

assets_C_sim_all_periods_ave = assets_C_sim_all_periods_num / assets_C_sim_all_periods_den
...
productivity_growth_S = zeros(num_S, period_all)

for period_i in 2:period_all
    for hh_i in 1:num_S
        if (shock_S_ρ[hh_i,period_i] == 1)
            productivity_growth_S[hh_i,period_i] = log(parameters.z_grid[shock_S_z[hh_i,period_i]]*parameters.η_grid[shock_S_η[hh_i,period_i]]) - log(parameters.z_grid[shock_S_z[hh_i,period_i-1]]*parameters.η_grid[shock_S_η[hh_i,period_i-1]])
        end
    end
end
...
y_S_prod = c_growth_S_new[(shock_S_ρ[:,2:end] .== 1)]

productivity_growth_S_new = productivity_growth_S[:,2:end]

productivity_growth_S_X = productivity_growth_S_new[(shock_S_ρ[:,2:end] .== 1)]

age_S_prod = panel_S_age[:,2:end]

age_S_X_prod = age_S_prod[(shock_S_ρ[:,2:end] .== 1)]

row_num = sum((shock_S_ρ[:,2:end] .== 1))
...
for period_i in 2:period_all
    for hh_i in 1:num_C
        if (shock_C_ρ[hh_i,period_i] == 1) && (shock_C_div[hh_i,period_i] == 2)
            c_growth_C[hh_i,period_i] = log(panel_C_c[hh_i,period_i]/2.0) - log(panel_C_c[hh_i,period_i-1]/2.0)
        end
    end
end

productivity_growth_C_1 = zeros(num_C, period_all)

for period_i in 2:period_all
    for hh_i in 1:num_C
        if (shock_C_ρ[hh_i,period_i] == 1) && (shock_C_div[hh_i,period_i] == 2)
            productivity_growth_C_1[hh_i,period_i] = log(parameters.z_grid[shock_C_z_1[hh_i,period_i]]*parameters.η_grid[shock_C_η_1[hh_i,period_i]]) - log(parameters.z_grid[shock_C_z_1[hh_i,period_i-1]]*parameters.η_grid[shock_C_η_1[hh_i,period_i-1]])
        end
    end
end

productivity_growth_C_2 = zeros(num_C, period_all)

for period_i in 2:period_all
    for hh_i in 1:num_C
        if (shock_C_ρ[hh_i,period_i] == 1) && (shock_C_div[hh_i,period_i] == 2)
            productivity_growth_C_2[hh_i,period_i] = log(parameters.z_grid[shock_C_z_2[hh_i,period_i]]*parameters.η_grid[shock_C_η_2[hh_i,period_i]]) - log(parameters.z_grid[shock_C_z_2[hh_i,period_i-1]]*parameters.η_grid[shock_C_η_2[hh_i,period_i-1]])
        end
    end
end
...
y_C_prod = c_growth_C_new[(shock_C_ρ[:,2:end] .== 1) .& (shock_C_div[:,2:end] .== 2)]

y_C_prod_new = vcat(y_C_prod,y_C_prod)

productivity_growth_C_1_new = productivity_growth_C_1[:,2:end]

productivity_growth_C_1_X = productivity_growth_C_1_new[(shock_C_ρ[:,2:end] .== 1) .& (shock_C_div[:,2:end] .== 2)]

productivity_growth_C_2_new = productivity_growth_C_2[:,2:end]

productivity_growth_C_2_X = productivity_growth_C_2_new[(shock_C_ρ[:,2:end] .== 1) .& (shock_C_div[:,2:end] .== 2)]

productivity_growth_C_X = vcat(productivity_growth_C_1_X,productivity_growth_C_2_X)

age_C = panel_C_age[:,2:end]

age_C_X_prod = age_C[(shock_C_ρ[:,2:end] .== 1) .& (shock_C_div[:,2:end] .== 2)]

age_C_X_prod_new = vcat(age_C_X_prod,age_C_X_prod)

row_num_C = sum((shock_C_ρ[:,2:end] .== 1) .& (shock_C_div[:,2:end] .== 2))*2

X_C_prod = zeros(row_num_C,4)

X_C_prod[:,1] .= 1.0
X_C_prod[:,2] = productivity_growth_C_X
X_C_prod[:,3] = age_C_X_prod_new
X_C_prod[:,4] = age_C_X_prod_new.^2

β_C_prod = (X_C_prod'*X_C_prod)\(X_C_prod'*y_C_prod_new)
