        ...
        Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
            ...
            for κ_p_i in 1:κ_size
                ...
                for κ_i in 1:κ_size, η_i in 1:η_size, z_i in 1:z_size
                    ...
                    @inbounds variables.μ_S[:,z_p_i,η_p_i,κ_p_i] .+= dropdims(sum(dropdims(sum(variables.σ_S[2:end,:,:,z_i,η_i,κ_i],dims=2),dims=2).*repeat(reshape(μ_S_p[:,z_i,η_i,κ_i].*(ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]),1,:),outer=[a_size 1]),dims=2),dims=2)

                    @inbounds variables.μ_S[a_ind_zero,z_p_i,η_p_i,κ_p_i] += ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*sum(dropdims(sum(variables.σ_S[1,:,:,z_i,η_i,κ_i],dims=1),dims=1).*μ_S_p[:,z_i,η_i,κ_i]) + (1-ρ)*Γ_η[η_p_i]*(z_p_i == 3)*(κ_p_i == 1)*sum(μ_S_p[:,z_i,η_i,κ_i])
                end

                # assign the result
                # @inbounds variables.μ_S[:,z_p_i,η_p_i,κ_p_i] .+= μ_temp
            end

            # Couples
            for κ_2_p_i in 1:κ_size, κ_1_p_i in 1:κ_size, η_2_p_i in 1:η_size, z_2_p_i in 1:z_size
                ...
                for κ_2_i in 1:κ_size, κ_1_i in 1:κ_size, η_2_i in 1:η_size, η_1_i in 1:η_size, z_2_i in 1:z_size, z_1_i in 1:z_size
                    ...
                    @inbounds variables.μ_C[:,z_p_i,z_2_p_i,η_p_i,η_2_p_i,κ_1_p_i,κ_2_p_i] .+= dropdims(sum(dropdims(sum(variables.σ_C[2:end,:,:,:,z_1_i,z_2_i,η_1_i,η_2_i,κ_1_i,κ_2_i],dims=[2 3]),dims = (2,3)).*repeat(reshape(μ_C_p[:,z_1_i,z_2_i,η_1_i,η_2_i,κ_1_i,κ_2_i].*(ρ*Γ_z[z_1_i,z_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_1_p_i]*Γ_κ[κ_2_p_i]),1,:),outer=[a_size 1]),dims=2),dims=2)

                    @inbounds variables.μ_C[a_ind_zero,z_p_i,z_2_p_i,η_p_i,η_2_p_i,κ_1_p_i,κ_2_p_i] += ρ*Γ_z[z_1_i,z_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_1_p_i]*Γ_κ[κ_2_p_i]*sum(dropdims(sum(variables.σ_C[1,:,:,:,z_1_i,z_2_i,η_1_i,η_2_i,κ_1_i,κ_2_i],dims=[1 2]),dims=(1,2)).*μ_C_p[:,z_1_i,z_2_i,η_1_i,η_2_i,κ_1_i,κ_2_i])+(1-ρ)*Γ_η[η_p_i]*Γ_η[η_2_p_i]*(z_p_i == 3)*(z_2_p_i == 3)*(κ_1_p_i == 1)*(κ_2_p_i == 1)*sum(μ_C_p[:,z_1_i,z_2_i,η_1_i,η_2_i,κ_1_i,κ_2_i])
                end

                # assign the result
                # @inbounds variables.μ_C[:,z_p_i,z_2_p_i,η_p_i,η_2_p_i,κ_1_p_i,κ_2_p_i] .+= μ_temp
            end
        end
