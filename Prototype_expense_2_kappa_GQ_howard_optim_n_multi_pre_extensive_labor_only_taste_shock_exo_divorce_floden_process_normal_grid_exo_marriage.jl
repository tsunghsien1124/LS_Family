...
function parameters_function(;
    ...
    Γ_z = [ 0.818028    0.178439  0.00353363;
            0.178439    0.643122  0.178439;
            0.00353363  0.178439  0.818028]
    ...
    z_grid=z_grid, z_size=z_size, Γ_z=Γ_z, η_grid=η_grid, η_size=η_size, Γ_η=Γ_η, BM_indices = BM_indices, n_grid=n_grid, n_size = n_size, d_size=d_size, ζ=ζ, L_ζ=L_ζ, ψ=ψ)
end
...
mutable struct MutableVariables
    ...
    """
    v_S_R::Array{Float64,5}
    v_S_D::Array{Float64,4}
    W_S_D::Array{Float64,3}
    W_S_R::Array{Float64,3}
    W_S::Array{Float64,3}
    # F_S::Array{Float64,6}
    σ_S_R::Array{Float64,5}
    ...
    v_C_R_hat_1::Array{Float64,8}
    v_C_R_hat_2::Array{Float64,8}
    v_C_D::Array{Float64,7}
    v_C_D_hat_1::Array{Float64,7}
    v_C_D_hat_2::Array{Float64,7}
    W_C_D::Array{Float64,5}
    W_C_R::Array{Float64,5}
    W_C::Array{Float64,5}
    W_C_hat_1::Array{Float64,5}
    W_C_hat_2::Array{Float64,5}
    # F_C::Array{Float64,6}
...
function variables_function(
    ...
    # unpack parameters
    @unpack  a_size, n_size, z_size, η_size, d_size, ρ, r, ν = parameters

    # Singles' value functions
    v_S_R = zeros(a_size, n_size, a_size, z_size, η_size)
    v_S_D = zeros(n_size, a_size, z_size, η_size)
    W_S_D = zeros(a_size, z_size, η_size)
    W_S_R = zeros(a_size, z_size, η_size)
    W_S = zeros(a_size, z_size, η_size)

    # Singles' feasible set
    # F_S = zeros(a_size, n_size, a_size, z_size, η_size, κ_size)

    # Singles' choice probabilities
    σ_S_R = ones(a_size, n_size, a_size, z_size, η_size) ./ (a_size*n_size)
    σ_S_D = ones(n_size, a_size, z_size, η_size) ./ n_size
    σ_S_d = ones(d_size, a_size, z_size, η_size) ./ d_size
    σ_S = ones(a_size+1, n_size, a_size, z_size, η_size) ./ ((a_size+1)*n_size)

    # Couples' value functions
    v_C_R = zeros(a_size, n_size, n_size, a_size, z_size, z_size, η_size, η_size)
    v_C_R_hat_1 = zeros(a_size, n_size, n_size, a_size, z_size, z_size, η_size, η_size)
    v_C_R_hat_2 = zeros(a_size, n_size, n_size, a_size, z_size, z_size, η_size, η_size)
    v_C_D = zeros(n_size, n_size, a_size, z_size, z_size, η_size, η_size)
    v_C_D_hat_1 = zeros(n_size, n_size, a_size, z_size, z_size, η_size, η_size)
    v_C_D_hat_2 = zeros(n_size, n_size, a_size, z_size, z_size, η_size, η_size)
    ...
    W_C_hat_1 = zeros(a_size, z_size, z_size, η_size, η_size)
    W_C_hat_2 = zeros(a_size, z_size, z_size, η_size, η_size)
    ...
    variables = MutableVariables(v_S_R, v_S_D, W_S_D, W_S_R, W_S, σ_S_R, σ_S_D, σ_S_d, σ_S, P_S, q_S, v_C_R, v_C_R_hat_1, v_C_R_hat_2, v_C_D, v_C_D_hat_1, v_C_D_hat_2, W_C_D, W_C_R, W_C, W_C_hat_1, W_C_hat_2, σ_C_R, σ_C_D, σ_C_d, σ_C, P_C, q_C, μ_S, μ_C)
    return variables
end
...
function value_function_singles!(
    ...
    @unpack  a_size, z_size, η_size, a_grid, z_grid, η_grid, T, θ, β, ρ, Γ_z, Γ_η, ϕ, a_ind_zero, BM_indices, α, n_grid, n_size, ζ, L_ζ, ψ = parameters

    W_expect_mat = reshape(W_S_p,a_size,:)*transpose(kron(reshape(parameters.Γ_η,1,:),parameters.Γ_z))

    W_expect_C_hat_1_mat = reshape(variables.W_C_hat_1,a_size,:)*transpose(kron(reshape(parameters.Γ_η,1,:),kron(reshape(parameters.Γ_η,1,:),kron(parameters.Γ_z,parameters.Γ_z))))

    variables.v_S_R .= -Inf
    variables.v_S_D .= -Inf

    for i in CartesianIndices((1:z_size,1:η_size))
        ...
        @inbounds z_C_i = LinearIndices(Γ_z)[z_i,z_i]
        ...
        for a_i in 1:a_size, n_i in 1:n_size
            ...
            if (a<0)
                ...
                if c <= 0.0
                    @inbounds variables.v_S_D[n_i, a_i, z_i, η_i] = -Inf
                else
                    @inbounds variables.v_S_D[n_i, a_i, z_i, η_i] = utility_function(c,l) + β*ρ*(1-ψ)*W_expect_mat[a_ind_zero,z_i] + β*ρ*ψ*W_expect_C_hat_1_mat[a_ind_zero,z_C_i]
                end
            end
        end
            ...
            for BM_i in 1:a_size
                ...
                if ub != 0
                    ...
                    for a_p_i in lb:ub
                        ...
                        if c <= 0.0
                            @inbounds variables.v_S_R[a_p_i,n_i, a_i, z_i, η_i] = -Inf
                        else
                            if a_p*2 in a_grid
                                a_p_i_double = findfirst(a_grid .== a_p*2)
                            elseif a_p < a_grid[1]
                                a_p_i_double = 1
                            else
                                a_p_i_double = a_size
                            ...
                            @inbounds variables.v_S_R[a_p_i,n_i, a_i, z_i, η_i] = utility_function(c,l) + β*ρ*(1-ψ)*W_expect_mat[a_p_i,z_i] + β*ρ*ψ*W_expect_C_hat_1_mat[a_p_i_double,z_C_i]
                        end
                    end

                    if all(variables.v_S_R[:,n_i,a_i,z_i,η_i] .== -Inf)
                        @inbounds BM_bounds[a_i,2] = 1
                        @inbounds BM_bounds[a_i,3] = 0
                    else
                        @inbounds @views U_star = maximum(variables.v_S_R[:,n_i,a_i,z_i,η_i])

                        # Get new bounds (in terms of asset grid index!)
                        @inbounds @views lb_new = findfirst(variables.v_S_R[:,n_i,a_i,z_i,η_i] .- U_star .>= L_ζ)
                        @inbounds @views ub_new = findlast(variables.v_S_R[:,n_i,a_i,z_i,η_i] .- U_star .>= L_ζ)
        ...
        for a_i in 1:a_size
            ...
            @inbounds @views v_S_R_max = maximum(variables.v_S_R[:,:,a_i,z_i,η_i])
            if v_S_R_max == -Inf
                @inbounds variables.W_S_R[a_i,z_i,η_i] = v_S_R_max
                # variables.σ_S_R[:,:,a_i,z_i,η_i,κ_i] .= 1.0/(a_size*n_size)
                @inbounds variables.σ_S_R[:,:,a_i,z_i,η_i] .= 0.0
            else
                @inbounds @views v_S_R_diff = variables.v_S_R[:,:,a_i,z_i,η_i] .- v_S_R_max
                v_S_R_exp = exp.(v_S_R_diff ./ ζ)
                v_S_R_sum = sum(v_S_R_exp)
                @inbounds variables.W_S_R[a_i,z_i,η_i] = v_S_R_max + ζ*log(v_S_R_sum)
                @inbounds variables.σ_S_R[:,:,a_i,z_i,η_i] = v_S_R_exp ./ v_S_R_sum
            end

            # compute defaut/repayment probabilities
            @inbounds d_vec = vcat(variables.W_S_D[a_i,z_i,η_i],variables.W_S_R[a_i,z_i,η_i])
            d_max = maximum(d_vec)
            d_diff = d_vec .- d_max
            d_exp = exp.(d_diff ./ ζ)
            d_sum = sum(d_exp)
            @inbounds variables.W_S[a_i,z_i,η_i] = d_max + ζ*log(d_sum)
            @inbounds variables.σ_S_d[:,a_i,z_i,η_i] = d_exp ./ d_sum

            @inbounds variables.σ_S[1,:,a_i,z_i,η_i] = variables.σ_S_d[1,a_i,z_i,η_i] .* variables.σ_S_D[:,a_i,z_i,η_i]
            @inbounds variables.σ_S[2:end,:,a_i,z_i,η_i] = variables.σ_S_d[2,a_i,z_i,η_i] .* variables.σ_S_R[:,:,a_i,z_i,η_i]
        end
    end
...
function value_function_couples!(
    ...
    W_C_hat_1_p::Array{Float64,5},
    W_C_hat_2_p::Array{Float64,5},
    variables::MutableVariables,
    parameters::NamedTuple;
    slow_updating::Real = 1.0
    )
    ...
    W_expect_C_hat_1_mat = reshape(W_C_hat_1_p,a_size,:)*transpose(kron(reshape(parameters.Γ_η,1,:),kron(reshape(parameters.Γ_η,1,:),kron(parameters.Γ_z,parameters.Γ_z))))

    W_expect_C_hat_2_mat = reshape(W_C_hat_2_p,a_size,:)*transpose(kron(reshape(parameters.Γ_η,1,:),kron(reshape(parameters.Γ_η,1,:),kron(parameters.Γ_z,parameters.Γ_z))))

    W_expect_S_mat = reshape(variables.W_S,a_size,:)*transpose(kron(reshape(parameters.Γ_η,1,:),parameters.Γ_z))

    variables.v_C_R .= -Inf
    variables.v_C_R_hat_1 .= -Inf
    variables.v_C_R_hat_2 .= -Inf
    variables.v_C_D .= -Inf
    variables.v_C_D_hat_1 .= -Inf
    variables.v_C_D_hat_2 .= -Inf

    variables.W_C_hat_1 .= 0.0
    variables.W_C_hat_2 .= 0.0
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        for a_i in 1:a_size, n_2_i in 1:n_size, n_1_i in 1:n_size
            ...
            if (a<0)
                ...
                if c <= 0.0
                    ...
                    @inbounds variables.v_C_D_hat_1[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = -Inf
                    @inbounds variables.v_C_D_hat_2[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = -Inf
                else
                    @inbounds variables.v_C_D[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = λ*utility_function(c/2,l_1) + (1-λ)*utility_function(c/2,l_2)+ β*ρ*(1-ψ)*W_expect_C_mat[a_ind_zero,z_i] + β*ρ*ψ*(λ*W_expect_S_mat[a_ind_zero, z_1_i] + (1-λ)*W_expect_S_mat[a_ind_zero, z_2_i])

                    @inbounds variables.v_C_D_hat_1[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = utility_function(c/2,l_1) + β*ρ*(1-ψ)*W_expect_C_hat_1_mat[a_ind_zero,z_i] + β*ρ*ψ*W_expect_S_mat[a_ind_zero, z_1_i]

                    @inbounds variables.v_C_D_hat_2[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = utility_function(c/2,l_2)+ β*ρ*(1-ψ)*W_expect_C_hat_2_mat[a_ind_zero,z_i] + β*ρ*ψ*W_expect_S_mat[a_ind_zero, z_2_i]
                end
            end
        end
                ...
                if ub != 0
                    ...
                    for a_p_i in lb:ub
                        ...
                        if c <= 0.0
                            ...
                            @inbounds variables.v_C_R_hat_1[a_p_i, n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = -Inf

                            @inbounds variables.v_C_R_hat_2[a_p_i, n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = -Inf
                        else
                            # if a_p_i < a_ind_zero
                            #     a_p_i_half = a_p_i + 1
                            ...
                            @inbounds variables.v_C_R[a_p_i, n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = λ*utility_function(c/2,l_1) + (1-λ)*utility_function(c/2,l_2)+ β*ρ*(1-ψ)*W_expect_C_mat[a_p_i,z_i] + β*ρ*ψ*(λ*W_expect_S_mat[a_p_i_half_1, z_1_i] + (1-λ)*W_expect_S_mat[a_p_i_half_2, z_2_i])

                            @inbounds variables.v_C_R_hat_1[a_p_i, n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = utility_function(c/2,l_1) + β*ρ*(1-ψ)*W_expect_C_hat_1_mat[a_p_i,z_i] + β*ρ*ψ*W_expect_S_mat[a_p_i_half_1, z_1_i]

                            @inbounds variables.v_C_R_hat_2[a_p_i, n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i] = utility_function(c/2,l_2)+ β*ρ*(1-ψ)*W_expect_C_hat_2_mat[a_p_i,z_i] + β*ρ*ψ*W_expect_S_mat[a_p_i_half_2, z_2_i]
                        end
                    end
        ...
        for a_i in 1:a_size
            ...
            for n_2_i in 1:n_size, n_1_i in 1:n_size
                if variables.σ_C[1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] > 0.0
                    @inbounds variables.W_C_hat_1[a_i,z_1_i,z_2_i,η_1_i,η_2_i] += variables.σ_C[1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] * variables.v_C_D_hat_1[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i]
                end

                for a_p_i in 1:a_size
                    if variables.σ_C[a_p_i+1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] > 0.0
                        @inbounds variables.W_C_hat_1[a_i,z_1_i,z_2_i,η_1_i,η_2_i] += variables.σ_C[a_p_i+1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] * variables.v_C_R_hat_1[a_p_i+1,n_1_i,n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i]
                    end
                end
            end
            ...
            for n_2_i in 1:n_size, n_1_i in 1:n_size
                if variables.σ_C[1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] > 0.0
                    @inbounds variables.W_C_hat_2[a_i,z_1_i,z_2_i,η_1_i,η_2_i] += variables.σ_C[1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] * variables.v_C_D_hat_2[n_1_i, n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i]
                end

                for a_p_i in 1:a_size
                    if variables.σ_C[a_p_i+1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] > 0.0
                        @inbounds variables.W_C_hat_2[a_i,z_1_i,z_2_i,η_1_i,η_2_i] += variables.σ_C[a_p_i+1,n_1_i,n_2_i,a_i,z_1_i,z_2_i,η_1_i,η_2_i] * variables.v_C_R_hat_2[a_p_i+1,n_1_i,n_2_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i]
                    end
                end
            end

            # @inbounds variables.W_C_hat_2[a_i,z_1_i,z_2_i,η_1_i,η_2_i] = sum(variables.σ_C[1,:,:,a_i,z_1_i,z_2_i,η_1_i,η_2_i] .* variables.v_C_D_hat_2[:, :, a_i, z_1_i, z_2_i, η_1_i, η_2_i]) + sum(variables.σ_C[2:end,:,:,a_i,z_1_i,z_2_i,η_1_i,η_2_i] .* variables.v_C_R_hat_2[:, :, :, a_i, z_1_i, z_2_i, η_1_i, η_2_i])
        end
    end
    end
    ...
    Threads.@threads for a_p_i in 1:(a_ind_zero-1)
        for z_i in 1:z_size
            ...
            for η_p_i in 1:η_size, z_p_i in 1:z_size
                @inbounds @views P_expect += Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*(1.0-sum(variables.σ_S[1,:,a_p_i,z_p_i,η_p_i]))
            end
    ...
    Threads.@threads for a_p_i in 1:(a_ind_zero-1)
        for z_2_i in 1:z_size, z_1_i in 1:z_size
            ...
            for η_2_p_i in 1:η_size, η_1_p_i in 1:η_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size
                @inbounds @views P_expect += Γ_z[z_1_i,z_1_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*(1.0-sum(variables.σ_C[1,:,:,a_p_i,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i]))
            end
        ...
        Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
            ...
            for a_p_i in 1:a_size
                ...
                for η_i in 1:η_size, z_i in 1:z_size, a_i in 1:a_size
                    ...
                    @inbounds @views temp = sum(variables.σ_S[1+a_p_i,:,a_i,z_i,η_i])
                    ...
                    if a_p_i == a_ind_zero
                        @inbounds @views temp_2 = sum(variables.σ_S[1,:,a_i,z_i,η_i])
                        if temp_2 > 0.0
                            ...
                            @inbounds μ_temp_survivor += ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*temp_2*μ_S_p[a_i,z_i,η_i]
                        end
...
function solve_function!(
    ...
    if one_loop == true
        ...
        W_C_hat_1_p = similar(variables.W_C_hat_1)
        W_C_hat_2_p = similar(variables.W_C_hat_2)
        ...
        while crit > tol && iter < iter_max
            ...
            copyto!(W_C_hat_1_p, variables.W_C_hat_1)
            copyto!(W_C_hat_2_p, variables.W_C_hat_2)
            ...
            value_function_couples!(W_C_p, W_C_hat_1_p, W_C_hat_2_p, variables, parameters; slow_updating = 1.0)
...
# cd("C:/Users/jsun/Dropbox/Bankruptcy-Family/Results/Current/Taste_shock_exo_divorce_0.1_Floden_process_normal_grid")
# @load "workspace.jld2"
