...
function parameters_function(;
    ...
    a_min::Real = -2.50,                        # minimum of assets
    a_max::Real = 8.00,                         # maximum of assets
    # a_degree::Real = 1.0,                      # governs how grid is spaced
    d_size::Integer = 2,                         # Number of default/repayment options
    T::Real = 1.2,
    α::Real = 0.35,                                 # Weight of leisure in utility function # From Alon et al. (2020)
    # α::Real = 0.2,
...
function utility_function(
    ...
    """
    @unpack T, α = parameters
    if (c > 0.0) && (l > 0.0)
        # return log(c) + α*log(l)
        return (c^(1.0-2.0))/(1.0-2.0) - α*(T - l)
    else
        return -Inf
    end
end
...
function pricing_function!(
    ...
    @unpack  a_size, z_size, η_size, a_grid, ρ, r, Γ_z, Γ_η, a_ind_zero, ψ = parameters
