...
function parameters_function(;
    ...
    n_grid=n_grid, n_size=n_size, z_grid=z_grid, z_size=z_size, Γ_z=Γ_z, η_grid=η_grid, η_size=η_size, Γ_η=Γ_η, κ_grid=κ_grid, κ_size = κ_size, Γ_κ = Γ_κ)
end
...
mutable struct MutableVariables
    ...
    n_S_i::Array{Int64,4}
    n_C_1_i::Array{Int64,6}
    n_C_2_i::Array{Int64,6}
    # l_S::Array{Float64,3}
    ...
    a_C_i::Array{Int64,6}
    d_S_i::Array{Int64,4}
    d_forced_S_i::Array{Int64,4}
    d_C_i::Array{Int64,6}
    d_forced_C_i::Array{Int64,6}
    P_S::Array{Float64,2}
    q_S::Array{Float64,2}
    P_C::Array{Float64,3}
    q_C::Array{Float64,3}
...
function variables_function(
    ...
    V_C = zeros(a_size, z_size, z_size, η_size, η_size, κ_size)
    ...
    # Labor
    n_S_i = zeros(Int64,a_size, z_size, η_size, κ_size)
    n_C_1_i = zeros(Int64,a_size, z_size, z_size, η_size, η_size, κ_size)
    n_C_2_i = zeros(Int64,a_size, z_size, z_size, η_size, η_size, κ_size)
    ...
    a_C_i = zeros(Int64,a_size, z_size, z_size, η_size, η_size, κ_size)

    # Default
    d_S_i = zeros(Int64,a_size, z_size, η_size, κ_size)
    d_forced_S_i = zeros(Int64,a_size, z_size, η_size, κ_size)
    d_C_i = zeros(Int64,a_size, z_size, z_size, η_size, η_size, κ_size)
    d_forced_C_i = zeros(Int64,a_size, z_size, z_size, η_size, η_size, κ_size)
    ...
    μ_C = ones(a_size, z_size, z_size, η_size, η_size, κ_size) ./ (a_size*z_size*z_size*η_size*η_size*κ_size)

    # return the outcome
    variables = MutableVariables(V_S, V_C, n_S_i, n_C_1_i, n_C_2_i, a_S_i, a_C_i, d_S_i, d_forced_S_i, d_C_i, d_forced_C_i, P_S, q_S, P_C, q_C, μ_S, μ_C)
    return variables
end
...
function value_function_singles!(
    ...
    @unpack  a_size, z_size, η_size, n_size, κ_size, a_grid, z_grid, η_grid, n_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero = parameters
    ...
    Threads.@threads for a_i in 1:a_size
        for  κ_i in 1:κ_size, η_i in 1:η_size, z_i in 1:z_size
            ...
            for a_p_i in 1:a_size, n_i in 1:n_size
                ...
                if v_temp > variables.V_S[a_i,z_i,η_i,κ_i]
                    @inbounds variables.V_S[a_i,z_i,η_i,κ_i] = v_temp
                    @inbounds variables.d_S_i[a_i,z_i,η_i,κ_i] = 1
                    @inbounds variables.a_S_i[a_i,z_i,η_i,κ_i] = a_p_i
                    @inbounds variables.n_S_i[a_i,z_i,η_i,κ_i] = n_i
                end
            end
        end
    end
    ...
    Threads.@threads for z_i in 1:z_size
        for κ_i in 1:κ_size, η_i in 1:η_size
            @inbounds z = z_grid[z_i]
            @inbounds η = η_grid[η_i]
            @inbounds κ = κ_grid[κ_i]
            ...
            for n_i in 1:n_size
                ...
                for a_i in 1:a_size
                    if v_temp > variables.V_S[a_i,z_i,η_i,κ_i]
                        ...
                        @inbounds variables.n_S_i[a_i,z_i,η_i,κ_i] = n_i
                        if v_temp == -1e100
                            @inbounds variables.d_forced_S_i[a_i,z_i,η_i,κ_i] = 1
                        end
                    end
                end
            end
...
function value_function_couples!(
    ...
    @unpack  a_size, z_size, η_size, n_size, κ_size, a_grid, z_grid, η_grid, n_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero, λ = parameters
    ...
    # variables.V_S_R .= -Inf
    Threads.@threads for a_i in 1:a_size
        for κ_i in 1:κ_size, η_2_i in 1:η_size, η_1_i in 1:η_size, z_2_i in 1:z_size, z_1_i in 1:z_size
            @inbounds a = a_grid[a_i]
            ...
            for a_p_i in 1:a_size, n_1_i in 1:n_size, n_2_i in 1:n_size
                ...
                @inbounds l_1 = T - n_1
                @inbounds l_2 = T - n_2
                @inbounds c = z_1*η_1*(n_1^θ)+z_2*η_2*(n_2^θ)+a-variables.q_C[a_p_i,z_1_i,z_2_i]*a_p-κ

                V_expect = 0.0

                for κ_p_i in 1:κ_size, η_2_p_i in 1:η_size, η_1_p_i in 1:η_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size
                    @inbounds V_expect += Γ_z[z_1_i,z_1_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_p_i]*V_C_p[a_p_i,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i,κ_p_i]
                end

                v_temp = λ*utility_function(c/2,l_1) + (1-λ)*utility_function(c/2,l_2)+ β*ρ*V_expect

                if v_temp > variables.V_C[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i]
                    @inbounds variables.V_C[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = v_temp
                    @inbounds variables.d_C_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = 1
                    @inbounds variables.a_C_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = a_p_i
                    @inbounds variables.n_C_1_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = n_1_i
                    @inbounds variables.n_C_2_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = n_2_i
                end
            end
        end
    end
    ...
    Threads.@threads for z_1_i in 1:z_size
        for κ_i in 1:κ_size, η_2_i in 1:η_size, η_1_i in 1:η_size, z_2_i in 1:z_size
            @inbounds z_1 = z_grid[z_1_i]
            @inbounds z_2 = z_grid[z_2_i]
            @inbounds η_1 = η_grid[η_1_i]
            @inbounds η_2 = η_grid[η_2_i]
            ...
            for n_1_i in 1:n_size, n_2_i in 1:n_size
                ...
                c = (z_1*η_1*(n_1^θ)+z_2*η_2*(n_2^θ))*(1.0-ϕ)-κ

                V_expect = 0.0

                for κ_p_i in 1:κ_size, η_2_p_i in 1:η_size, η_1_p_i in 1:η_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size
                    @inbounds V_expect += Γ_z[z_1_i,z_1_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_p_i]*V_C_p[a_ind_zero,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i,κ_p_i]
                end
                ...
                for a_i in 1:a_size
                    if v_temp > variables.V_C[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i]
                        @inbounds variables.V_C[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = v_temp
                        @inbounds variables.d_C_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = 2
                        @inbounds variables.a_C_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = a_ind_zero
                        @inbounds variables.n_C_1_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = n_1_i
                        @inbounds variables.n_C_2_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = n_2_i
                        if v_temp == -1e100
                            @inbounds variables.d_forced_C_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] = 1
                        end
                    end
                end
            end
...
function pricing_function!(
    ...
    Threads.@threads for a_p_i in 1:(a_ind_zero-1)
        for z_2_i in 1:z_size, z_1_i in 1:z_size
            ...
            for κ_p_i in 1:κ_size, η_2_p_i in 1:η_size, η_1_p_i in 1:η_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size
                @inbounds temp += Γ_z[z_1_i,z_1_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_p_i]*(variables.d_C_i[a_p_i,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i,κ_p_i] == 1)
            end
    ...
    while crit_μ > tol && iter_μ < iter_max
        ...
        Threads.@threads for a_p_i in 1:a_size
            ...
            for κ_p_i in 1:κ_size, η_2_p_i in 1:η_size, η_1_p_i in 1:η_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size
                ...
                for κ_i in 1:κ_size, η_2_i in 1:η_size, η_1_i in 1:η_size, z_2_i in 1:z_size, z_1_i in 1:z_size, a_i in 1:a_size

                    @inbounds μ_temp_survivor += ρ*Γ_z[z_1_i,z_1_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_p_i]*(variables.a_C_i[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i] == a_p_i)*μ_C_p[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i]
                    if a_p_i == a_ind_zero
                        @inbounds μ_temp_newborn += (1-ρ)*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*(z_1_p_i == 3)*(z_2_p_i == 3)*(κ_p_i == 1)*μ_C_p[a_i,z_1_i,z_2_i,η_1_i,η_2_i,κ_i]
                    end
                end

                # assign the result
                @inbounds variables.μ_C[a_p_i,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i,κ_p_i] += μ_temp_survivor + μ_temp_newborn
            end
        end
...
d_forced_S_i = variables.d_forced_S_i
d_C_i = variables.d_C_i
d_forced_C_i = variables.d_forced_C_i
P_S = variables.P_S
q_S = variables.q_S
P_C = variables.P_C
q_C = variables.q_C
...
using JLD2
@save "workspace.jld2" V_S V_C n_S_i n_C_1_i n_C_2_i a_S_i a_C_i d_S_i d_forced_S_i d_C_i d_forced_C_i P_S q_S P_C q_C μ_S μ_C
