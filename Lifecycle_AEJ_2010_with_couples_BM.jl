        ...
        for κ_i in 1:κ_size
            ...
            for BM_i in 1:a_size
                ...
                if ub != 0
                    ...
                    if all(variables.v_div_R[:,a_i,z_i,η_i,κ_i,age] .== -Inf)
                        @inbounds BM_bounds[a_i,2] = 1
                        @inbounds BM_bounds[a_i,3] = 0
                    else
                        @inbounds @views U_star = maximum(variables.v_div_R[:,a_i,z_i,η_i,κ_i,age])

                        # Get new bounds (in terms of asset grid index!)
                        @inbounds @views lb_new = findfirst(variables.v_div_R[:,a_i,z_i,η_i,κ_i,age] .- U_star .>= L_ζ)
                        @inbounds @views ub_new = findlast(variables.v_div_R[:,a_i,z_i,η_i,κ_i,age] .- U_star .>= L_ζ)
