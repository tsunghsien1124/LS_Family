# Prototype model, version where optim algos were changed to gradient-using, outdated, use multi-grad instead
...
function Util_R_C_optim_function(
    ...
    if (c > 0.0) && (l_1 > 0.0) && (l_2 > 0.0)
        grad[1] = θ*z_1*η_1*(n_1^(θ-1.0))/(z_1*η_1*(n_1^θ)+z_2*η_2*(n_2^θ)+a-q_C*a_p-κ_1-κ_2)-(λ*α/(T-n_1))
        grad[2] = θ*z_2*η_2*(n_2^(θ-1.0))/(z_1*η_1*(n_1^θ)+z_2*η_2*(n_2^θ)+a-q_C*a_p-κ_1-κ_2)-((1-λ)*α/(T-n_2))
...
function Util_D_C_optim_function(
    ...
    if (c > 0.0) && (l_1 > 0.0) && (l_2 > 0.0)
        grad[1] = θ*z_1*η_1*(n_1^(θ-1.0))/(z_1*η_1*(n_1^θ)+z_2*η_2*(n_2^θ))-(λ*α/(T-n_1))
        grad[2] = θ*z_2*η_2*(n_2^(θ-1.0))/(z_1*η_1*(n_1^θ)+z_2*η_2*(n_2^θ))-((1.0-λ)*α/(T-n_2))
        return λ*log(c/2) + (1-λ)*log(c/2) + λ*α*log(l_1) + (1-λ)*α*log(l_2) + β*ρ*V_expect
    else
        return -Inf
    end
        ...
        for BM_i in 1:a_size
            ...
            if ub != 0
                ...
                for a_p_i in lb:ub #, n_i in 1:n_size
                    ...
                    n_min = max(((q_S*a_p+κ-a)/(z*η)),0.001)^(1/θ)
