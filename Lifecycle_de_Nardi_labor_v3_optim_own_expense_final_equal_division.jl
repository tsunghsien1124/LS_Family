...
# 0.00024362075390	0.00752487973592	0.05603155653834	0.23546001015495	0.70073993281691]
    ...
    # z_grid[:,2] = [0.33417655181070505, 0.5780800565758215, 1.0, 1.7298642093335026, 2.992430182733024] # Men
        z_grid[:,2] = [0.4155809704471233, 0.6446556991504251, 1.0, 1.5512156354436546, 2.4062699476448612] # Women
    ...
    #             2.41382e-6  0.000235301  0.0086015  0.139747     0.851414] # Men
                Γ_z[:,:,2] = [ 0.802665     0.181378     0.0153698  0.000578853  8.17523e-6;
                            0.0453446    0.81035      0.136468   0.00769308   0.000144713;
                            0.00256163   0.0909786    0.812919   0.0909786    0.00256163;
                            0.000144713  0.00769308   0.136468   0.81035      0.0453446;
                            8.17523e-6   0.000578853  0.0153698  0.181378     0.802665] # Women
                ...
                for n_2_i in 1:n_size, n_1_i in 1:n_size, a_p_i in 1:a_size
                    ...
                    if (c <= 0.0) || (variables.q_C[a_p_i,z_1_i,z_2_i,age] ≈ 0.0)
                        ...
                        if age == lifespan
                            ...
                            @inbounds temp = utility_function(c/es_grid[age],l_1) + utility_function(c/es_grid[age],l_2)+ β*(1-ψ)*V_expect_C_mat[a_p_i,z_i] + β*ψ*(0.5*(V_expect_div_mat_women[a_p_i_half_1, z_1_i] + V_expect_div_mat_men[a_p_i_half_2, z_2_i])+0.5*(V_expect_div_mat_women[a_p_i_half_2, z_1_i] + V_expect_div_mat_men[a_p_i_half_1, z_2_i]))
                        end
                    end
    ...
    if age == lifespan
        ...
        Threads.@threads for a_p_i in 1:(a_ind_zero-1)
            ...
            for z_2_i in 1:z_size, z_1_i in 1:z_size
                P_expect_remain = 0.0
                P_expect_div_1 = 0.0
                P_expect_div_1_alt = 0.0
                P_expect_div_2 = 0.0
                P_expect_div_2_alt = 0.0
                ...
                for κ_2_p_i in 1:κ_size, κ_1_p_i in 1:κ_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size
                    ...
                    if variables.d_C_i[a_p_i,z_1_p_i,z_2_p_i,κ_1_p_i,κ_2_p_i,age+1] == 2 # Default
                            ...
                            # recovery = (parameters.ϕ)/(abs(a_grid[a_p_i])+κ_grid[age+1,1,κ_1_p_i]+κ_grid[age+1,1,κ_2_p_i]) # 1 is for couples
                ...
                for κ_1_p_i in 1:κ_size, z_1_p_i in 1:z_size
                    ...
                    if variables.d_div_i[a_p_i_half_2,z_1_p_i,κ_1_p_i,age+1,1] == 2 # Default
                        n = n_grid[variables.n_div_i[a_p_i_half_2,z_1_p_i,κ_1_p_i,age+1,1]]

                        recovery = ((z_grid[z_1_p_i,1]*h_grid[age+1]*n)*parameters.ϕ)/(abs(a_grid[a_p_i_half_2])+κ_grid[age+1,2,κ_1_p_i]+κ_div) # 2 is for singles
                        # recovery = (parameters.ϕ)/(abs(a_grid[a_p_i_half_1])+κ_grid[age+1,2,κ_1_p_i]+κ_div) # 2 is for singles
                        ...
                        P_expect_div_1_alt += Γ_z[z_1_i,z_1_p_i,1]*Γ_κ[κ_1_p_i]*recovery

                    else
                        P_expect_div_1_alt += Γ_z[z_1_i,z_1_p_i,1]*Γ_κ[κ_1_p_i]
                    end
                end
                ...
                for κ_2_p_i in 1:κ_size, z_2_p_i in 1:z_size
                    ...
                    if variables.d_div_i[a_p_i_half_1,z_2_p_i,κ_2_p_i,age+1,2] == 2 # Default
                        n = n_grid[variables.n_div_i[a_p_i_half_1,z_2_p_i,κ_2_p_i,age+1,2]]

                        recovery = ((z_grid[z_2_p_i,2]*h_grid[age+1]*n)*parameters.ϕ)/(abs(a_grid[a_p_i_half_1])+κ_grid[age+1,2,κ_2_p_i]+κ_div) # 2 is for singles
                        # recovery = (parameters.ϕ)/(abs(a_grid[a_p_i_half_2])+κ_grid[age+1,2,κ_2_p_i]+κ_div) # 2 is for singles
                        ...
                        P_expect_div_2_alt += Γ_z[z_2_i,z_2_p_i,2]*Γ_κ[κ_2_p_i]*recovery
                    else
                        P_expect_div_2_alt += Γ_z[z_2_i,z_2_p_i,2]*Γ_κ[κ_2_p_i]
                    end
                end

                @inbounds variables.P_C[a_p_i,z_1_i,z_2_i,age] = (1.0-ψ)*P_expect_remain + ψ*(0.5*((a_p_half_1/a_p)*P_expect_div_1 + (a_p_half_2/a_p)*P_expect_div_2)+0.5*((a_p_half_2/a_p)*P_expect_div_1_alt + (a_p_half_1/a_p)*P_expect_div_2_alt))
...
# cd("C:/Users/jsun/Dropbox/Bankruptcy-Family/Results/Current/Calibration-14")
cd("C:/Users/jsun/Dropbox/Bankruptcy-Family/Results/Current/Calibration-14/Equal_division/phi-0.9")
# cd("C:/Users/jsun/Dropbox/Bankruptcy-Family/Results/Current/Calibration-14/Equal_division_same_income/phi-0.9")
