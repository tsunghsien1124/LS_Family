...
plot_n_S = plot!(parameters.a_grid,n_S[:,1,η_ind],legend=:bottomleft,label="\$ z=$(round(parameters.z_grid[1],digits=2)) \$", lw = 3, lc= :blue, xlabel = L"a", title="\$ \\textrm{Labor supply choice for Singles } n_S(a,z,\\eta=$(round(parameters.η_grid[η_ind],digits=2))) \$")
...
plot_n_S = plot!(parameters.a_grid,n_S[:,2,η_ind], label="\$ z=$(round(parameters.z_grid[2],digits=2)) \$",lw = 3, lc = :green)
...
plot_n_S = plot!(parameters.a_grid,n_S[:,3,η_ind], label="\$ z=$(round(parameters.z_grid[3],digits=2)) \$",lw = 3, lc = :black)
