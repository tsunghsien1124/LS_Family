...
function parameters_function(;
    ...
    z_grid=z_grid, z_size=z_size, Γ_z=Γ_z, η_grid=η_grid, η_size=η_size, Γ_η=Γ_η, BM_indices = BM_indices, d_size=d_size, ζ=ζ, L_ζ=L_ζ, ψ=ψ, μ_asset_newborns=μ_asset_newborns)
end
...
function variables_function(
    ...
    # unpack parameters
    @unpack  a_size, z_size, η_size, κ_size, d_size, ρ, r, ν = parameters

    # Singles' value functions
    v_S_R = zeros(a_size, a_size, z_size, η_size, κ_size)
    W_S_D = zeros(z_size, η_size)
    ...
    σ_S_R = ones(a_size, a_size, z_size, η_size, κ_size) ./ (a_size)
    σ_S_d = ones(d_size, a_size, z_size, η_size, κ_size) ./ d_size
    σ_S = ones(a_size+1, a_size, z_size, η_size, κ_size) ./ (a_size+1)
...
function value_function_singles!(
    ...
    @unpack  a_size, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero, BM_indices, α, ζ, L_ζ = parameters
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        #     W_expect += Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*W_S_p[a_ind_zero,z_p_i,η_p_i,κ_p_i]
        # end
        ...
        if c <= 0.0
            @inbounds variables.W_S_D[z_i, η_i] = -Inf
        else
            @inbounds variables.W_S_D[z_i, η_i] = utility_function(c) + β*ρ*W_expect_mat[a_ind_zero,z_i]
            # @inbounds variables.W_S_D[z_i, η_i] = utility_function(c) + β*ρ*W_expect
        end
        ...
        for κ_i in 1:κ_size
            ...
            for a_i in 1:a_size
                ...
                for a_p_i in 1:a_size
                    ...
                    @inbounds c = z*η*(1.0^(θ))+a-variables.q_S[a_p_i,z_i]*a_p-κ

                    # W_expect = 0.0
                    #
                    # for z_p_i in 1:z_size, η_p_i in 1:η_size, κ_p_i in 1:κ_size
                    #     W_expect += Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*W_S_p[a_p_i,z_p_i,η_p_i,κ_p_i]
                    # end

                    if c <= 0.0
                        @inbounds variables.v_S_R[a_p_i, a_i, z_i, η_i, κ_i] = -Inf
                    else
                        @inbounds variables.v_S_R[a_p_i, a_i, z_i, η_i, κ_i] = utility_function(c) + β*ρ*W_expect_mat[a_p_i,z_i]
                        # @inbounds variables.v_S_R[a_p_i, a_i, z_i, η_i, κ_i] = utility_function(c) + β*ρ*W_expect
                    end
                end
            end
        end
    ...
    # Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        for a_i in 1:a_size
            ...
            for κ_i in 1:κ_size
                ...
                @inbounds d_vec = vcat(variables.W_S_D[z_i,η_i],variables.W_S_R[a_i,z_i,η_i,κ_i])
                d_max = maximum(d_vec)
                d_diff = d_vec .- d_max
                d_exp = exp.(d_diff ./ ζ)
                d_sum = sum(d_exp)
                ...
                @inbounds variables.σ_S[1,a_i,z_i,η_i,κ_i] = variables.σ_S_d[1,a_i,z_i,η_i,κ_i]
...
function pricing_function!(
    ...
    @unpack  a_size, z_size, z_grid, η_size, η_grid, κ_grid, κ_size, a_grid, ρ, r, Γ_z, Γ_η, Γ_κ, a_ind_zero, ψ = parameters
    ...
    Threads.@threads for a_p_i in 1:(a_ind_zero-1)
        for z_i in 1:z_size
            ...
            for κ_p_i in 1:κ_size, η_p_i in 1:η_size, z_p_i in 1:z_size
                ...
                @inbounds @views P_expect += Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*((1.0-variables.σ_S[1,a_p_i,z_p_i,η_p_i,κ_p_i])+(variables.σ_S[1,a_p_i,z_p_i,η_p_i,κ_p_i]*recovery))
            end
