...
function parameters_function(;
    ...
    a_grid = collect(range(a_min, a_max, step=0.1))
    ...
    z_grid=z_grid, z_size=z_size, Γ_z=Γ_z, η_grid=η_grid, η_size=η_size, Γ_η=Γ_η, κ_grid=κ_grid, κ_size = κ_size, Γ_κ = Γ_κ, BM_indices = BM_indices, n_grid=n_grid, n_size = n_size)
end
...
function value_function_singles!(
    ...
    @unpack  a_size, a_min, a_max, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero, BM_indices, α, n_grid, n_size = parameters
        ...
        for BM_i in 1:a_size
            ...
            if ub != 0
                ...
                for n_i in 1:n_size
                    ...
                    #     l = T - n
                    #
                    #     c = z*η*(n^(θ))+a-variables.q_S[a_p_i,z_i]*a_p-κ
                    #     v_temp[a_p_i] = utility_function(c,l) + β*ρ*V_expect_mat[a_p_i,z_i]
                    # end
                    ...
                    for a_p_i in lb:ub
                        ...
                        a_p_temp[a_p_i] = utility_function(c,l) + β*ρ*V_expect
                    ...
                    # else
                        # interp = CubicSplineInterpolation(x_test, v_temp)

                        # interp = LinearInterpolation(a_grid, v_temp)

                        # interp = LinearInterpolation(x_test[a_p_temp .!= -Inf], a_p_temp[a_p_temp .!= -Inf])
                        # interp = LinearInterpolation(x_test, v_temp)

                        # test_1 = findfirst(a_p_temp .!= -Inf)
                        # test_2 = findlast(a_p_temp .!= -Inf)

                        # if any(a_p_temp[test_1:test_2] .== -Inf)
                        #     error("Non-monotonic -Inf")
                        # end
                        #
                        # if a_p_i_lower < test_1
                        #     a_p_lower = a_grid[test_1]
                        # end
                        #
                        # if a_p_i_upper > test_2
                        #     a_p_upper = a_grid[test_2]
                        # end

                        # try
                        l = T - n

                        opt = optimize(x -> -(utility_function(z*η*(n^(θ))+a-interp_q(x)*x-κ,l)+β*ρ*interp_V(x)), a_p_lower, a_p_upper, GoldenSection())
                        # catch
                        #     test_1 = findfirst(a_p_temp .!= -Inf)
                        #     test_2 = findlast(a_p_temp .!= -Inf)
                        #     println("$test_1")
                    ...
                    # f(a_p::Vector,grad::Vector) = spl(a_p)
                    #
                    # opt = Opt(:LN_COBYLA,1)
                    # opt.max_objective = f
                    # opt.lower_bounds = a_p_lower
                    # opt.upper_bounds = a_p_upper
                    # opt.xtol_rel = 1e-4
                    #
                    # (optf,optx,ret) = optimize(opt,[a_grid[a_p_i_max]])
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        @inbounds η_i = i[2]
            ...
            for n_i in 1:n_size
                ...
                optf = utility_function(c,l) + β*ρ*V_expect
