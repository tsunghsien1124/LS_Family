...
function parameters_function(;
    ...
    a_size_neg::Integer = 10,                   # number of  assets
...
function value_function_singles!(
    ...
    @unpack  a_size, a_min, a_max, z_size, η_size, a_grid, z_grid, η_grid, T, θ, β, ρ, Γ_z, Γ_η, ϕ, a_ind_zero, BM_indices, α, n_grid, n_size, x_integration = parameters
    ...
    # interp_V_S_R = LinearInterpolation((a_grid,z_grid,η_grid),variables.V_S_R,extrapolation_bc=Line())
    #
    # interp_V_S_D = LinearInterpolation((z_grid,η_grid),variables.V_S_D,extrapolation_bc=Line())
    #
    # function interp_V(a_p,z,η)
    #
    #     if isnan(interp_V_S_R(a_p,z,η))
    #         a_p_i = findfirst(a_grid .≈ a_p)
    #
    #         interp_V_S_R_2 = LinearInterpolation((z_grid,η_grid),variables.V_S_R[a_p_i,:,:],extrapolation_bc=Line())
    #
    #         return max(interp_V_S_D(z,η),interp_V_S_R_2(z,η))
    #     else
    #         return max(interp_V_S_D(z,η),interp_V_S_R(a_p,z,η))
    #     end
    #
    # end
    ...
    V_expect_R_mat = zeros(a_size,z_size)
    V_expect_D_mat = zeros(z_size)
    ...
    Threads.@threads for z_i in 1:z_size
        ...
        for a_p_i in 1:a_size
            ...
            interp_V_S_R = LinearInterpolation((z_grid,η_grid),variables.V_S_R[a_p_i,:,:],extrapolation_bc=Line())

            # f(x) = interp_V(a_p,x[1],x[2])*pdf(Normal(ρ*z,0.0426),x[1])*pdf(Normal(0.0,0.0421),x[2])

            f_S_R(x) = interp_V_S_R(exp(x[1] + ρ*log(z)),exp(x[2]))*pdf(Normal(0.0,0.0426),x[1])*pdf(Normal(0.0,0.0421),x[2])

            # (V_expect,err) = hcubature(f::Function, x_min, x_max; reltol=1e-8, abstol=0, maxevals=0)

            (V_expect_S_R,err) = hcubature(f_S_R::Function, x_min, x_max; abstol=1e-8, maxevals=0)

            V_expect_R_mat[a_p_i,z_i] = V_expect_S_R
        end

        interp_V_S_D = LinearInterpolation((z_grid,η_grid),variables.V_S_D[:,:],extrapolation_bc=Line())

        f_S_D(x) = interp_V_S_D(exp(x[1] + ρ*log(z)),exp(x[2]))*pdf(Normal(0.0,0.0426),x[1])*pdf(Normal(0.0,0.0421),x[2])

        (V_expect_S_D,err) = hcubature(f_S_D::Function, x_min, x_max; abstol=1e-8, maxevals=0)

        V_expect_D_mat[z_i] = V_expect_S_D
    end
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        # interp_V_expect = LinearInterpolation(a_grid,V_expect_mat[:,z_i])
        
        interp_V_expect_R = LinearInterpolation(a_grid,V_expect_R_mat[:,z_i])

        interp_V_expect_D = LinearInterpolation(a_grid,V_expect_D_mat[:,z_i])
            ...
            if ub != 0
                ...
                for n_i in 1:n_size
                    ...
                    for a_p_i in lb:ub
                        ...
                        a_p_temp[a_p_i] = utility_function(c,l) + β*ρ*V_expect_mat[a_p_i,z_i]
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        @inbounds η_i = i[2]
            ...
            for n_i in 1:n_size
                ...
                optf = utility_function(c,l) + β*ρ*V_expect_mat[a_ind_zero,z_i]
...
function howard_singles!(
    ...
    @unpack  a_size, a_min, a_max, z_size, η_size, a_grid, z_grid, η_grid, T, θ, β, ρ, Γ_z, Γ_η, ϕ, a_ind_zero, BM_indices, x_integration = parameters
        ...
        Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
            ...
            for a_i in 1:a_size
                ...
                if variables.d_S_i[a_i,z_i,η_i] == 1
                    ...
                    @inbounds variables.V_S[a_i,z_i,η_i] = utility_function(c,l) + β*ρ*V_expect
                end
            end
        end
        iter += 1
...
stationary_distribution!(variables, parameters; tol = 1E-5, iter_max = 5000)
