    ...
    while crit_μ > tol && iter_μ < iter_max
        ...
        Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
            ...
            for κ_p_i in 1:κ_size
                ...
                μ_temp_survivor = zeros(a_size)
                # μ_temp_newborn = 0.0

                for κ_i in 1:κ_size, η_i in 1:η_size, z_i in 1:z_size

                    @inbounds @views μ_temp_survivor .+= dropdims(sum(dropdims(sum(variables.σ_S[2:end,:,:,z_i,η_i,κ_i],dims=2),dims=2).*repeat(reshape(μ_S_p[:,z_i,η_i,κ_i].*(ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]),1,:),outer=[a_size 1]),dims=2),dims=2)

                    @inbounds @views μ_temp_survivor[a_ind_zero] += ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*sum(dropdims(sum(variables.σ_S[1,:,:,z_i,η_i,κ_i],dims=1),dims=1).*μ_S_p[:,z_i,η_i,κ_i]) + (1-ρ)*Γ_η[η_p_i]*(z_p_i == 3)*(κ_p_i == 1)*sum(μ_S_p[:,z_i,η_i,κ_i])
                end

                # assign the result
                @inbounds variables.μ_S[:,z_p_i,η_p_i,κ_p_i] .+= μ_temp_survivor
            end
