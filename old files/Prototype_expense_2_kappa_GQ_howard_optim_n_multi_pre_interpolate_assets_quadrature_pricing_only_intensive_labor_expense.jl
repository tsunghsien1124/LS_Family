...
function parameters_function(;
    ...
    # return the outcome
    return (β=β, λ=λ, r=r, ρ=ρ, T=T, α=α, γ_c=γ_c, γ_l=γ_l, ϕ=ϕ, χ=χ, θ=θ, ν=ν, κ_grid = κ_grid, Γ_κ = Γ_κ, κ_size = κ_size,
...
function variables_function(
    ...
    # unpack parameters
    @unpack  a_size, a_size_dist, z_size, η_size, ρ, r, ν, κ_size = parameters

    # Singles' value functions
    V_S = zeros(a_size, z_size, η_size, κ_size)
    V_S_R = zeros(a_size, z_size, η_size, κ_size)
    ...
    # Labor
    n_S = ones(Float64,a_size, z_size, η_size, κ_size).*0.5
    ...
    # Asset
    a_S = zeros(Float64,a_size, z_size, η_size, κ_size)
    ...
    μ_S = ones(a_size_dist, z_size, η_size, κ_size) ./ (a_size_dist*z_size*η_size*κ_size)
...
function value_function_singles!(
    ...
    @unpack  a_size, a_min, a_max, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, χ, a_ind_zero, BM_indices, α, n_grid, n_size, x_integration = parameters
    ...
    # V_expect_mat = reshape(V_S_p,a_size,:)*transpose(kron(reshape(Γ_η,1,:),Γ_z))
            ...
            if ub != 0
                ...
                for n_i in 1:n_size
                    ...
                    if !isinf(maximum(value_temp))
                        ...
                        f(x) = -(utility_function(z*η*(x[1]^(θ))+a-interp_q(x[2])*x[2]-κ,T-x[1])+β*ρ*interp_V_expect(x[2]))
                        ...
                        if optf > variables.V_S_R[a_i,z_i,η_i,κ_i]
                            variables.V_S_R[a_i,z_i,η_i,κ_i] = optf
                        end
            ...
            if !isinf(maximum(value_temp))
                ...
                for κ_i in 1:κ_size, a_i in 1:a_size
                    if optf > variables.V_S[a_i,z_i,η_i,κ_i]
                        ...
                        @inbounds variables.a_S[a_i,z_i,η_i,κ_i] = a_grid[a_ind_zero]
                        # @inbounds variables.n_S_i[a_i,z_i,η_i,κ_i] = optx[1]
                        @inbounds variables.n_S[a_i,z_i,η_i,κ_i] = optx_n
                    end
                end
...
function howard_singles!(
    ...
    @unpack  a_size, a_min, a_max, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, χ, a_ind_zero, BM_indices, x_integration = parameters
    ...
    while iter <= howard_iter
        ...
        V_expect_mat = reshape(variables.V_S,a_size,:)*transpose(kron(reshape(Γ_κ,1,:),kron(reshape(Γ_η,1,:),Γ_z)))
        ...
        Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
            ...
            for κ_i in 1:κ_size
                ...
                for a_i in 1:a_size
                    ...
                    @inbounds a_p = variables.a_S[a_i,z_i,η_i,κ_i]
                    # @inbounds a_p = a_grid[a_p_i]
                    @inbounds n = variables.n_S[a_i,z_i,η_i,κ_i]

                    if variables.d_S_i[a_i,z_i,η_i,κ_i] == 1
                        l = T - n
                        @inbounds c = z*η*(n^(θ))+a-interp_q(a_p)*a_p - κ
                        ...
                        @inbounds variables.V_S[a_i,z_i,η_i,κ_i] = utility_function(c,l) + β*ρ*interp_V(a_p)
                    else
                        l = T - n
                        # c = z*η*(n^(θ))-ϕ
                        c = (z*η*(n^(θ)))*(1.0-ϕ)
                        ...
                        @inbounds variables.V_S[a_i,z_i,η_i,κ_i] = utility_function(c,l) - χ + β*ρ*V_expect
                    end
                end
            end
        end
...
function pricing_function!(
    ...
    @unpack  z_grid, η_grid, κ_grid, a_size, z_size, η_size, κ_size, ρ, r, Γ_z, Γ_η, Γ_κ, a_ind_zero, x_integration = parameters
    ...
    Threads.@threads for a_p_i in 1:(a_ind_zero-1)
        ...
        for z_i in 1:z_size
            ...
            #
            #     interp_V_S_R = LinearInterpolation((z_grid,η_grid),variables.V_S_R[a_p_i,:,:,κ_i],extrapolation_bc=Interpolations.Line())
            #
            ...
            #
            #     R_expect_sum += Γ_κ[κ_i]*R_expect
            # end

            R_expect_sum = 0.0

            for κ_p_i in 1:κ_size, η_p_i in 1:η_size, z_p_i in 1:z_size
                @inbounds R_expect_sum += Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*(variables.d_S_i[a_p_i,z_p_i,η_p_i,κ_p_i] == 1)
            end

            @inbounds variables.P_S[a_p_i,z_i] = R_expect_sum

            @inbounds variables.q_S[a_p_i,z_i] = ρ*R_expect_sum/(1+r)
        end
    end
...
function stationary_distribution!(
    ...
    # Unpack grids
    @unpack  a_size_dist, z_size, η_size, κ_size, Γ_z, Γ_η, Γ_κ, a_grid, a_grid_dist, a_ind_zero_dist, ρ = parameters
                ...
                for a_p_i in 1:a_size_dist
                    ...
                    for κ_i in 1:κ_size, η_i in 1:η_size, z_i in 1:z_size
                        ...
                        for a_i in 1:a_size_dist
                            ...
                            lower = findlast(variables.a_S[a_i,z_i,η_i,κ_i] .>= a_grid_dist)
                            upper = findfirst(variables.a_S[a_i,z_i,η_i,κ_i] .<= a_grid_dist)
                            ...
                            elseif upper == lower
                                temp = 1.0
                            elseif a_p_i == lower
                                temp = (a_grid_dist[upper] - variables.a_S[a_i,z_i,η_i,κ_i])/(a_grid_dist[upper]-a_grid_dist[lower])
                            elseif a_p_i == upper
                                temp = (variables.a_S[a_i,z_i,η_i,κ_i] - a_grid_dist[lower])/(a_grid_dist[upper]-a_grid_dist[lower])
                            end
