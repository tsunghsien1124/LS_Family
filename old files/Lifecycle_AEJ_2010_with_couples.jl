...
0.00024362075390	0.00752487973592	0.05603155653834	0.23546001015495	0.70073993281691]
    ...
    z_grid=z_grid, z_size=z_size, Γ_z=Γ_z, Γ_z_initial=Γ_z_initial, η_grid=η_grid, η_size=η_size, Γ_η=Γ_η, BM_indices = BM_indices, d_size=d_size, ζ=ζ, L_ζ=L_ζ, ψ=ψ, μ_asset_newborns=μ_asset_newborns, lifespan = lifespan, h_grid=h_grid, es_grid=es_grid, κ_div=κ_div)
end
...
mutable struct MutableVariables
    ...
    P_S::Array{Float64,3}
    q_S::Array{Float64,3}
    v_div_R::Array{Float64,6}
    # v_div_D::Array{Float64,4}
    W_div_D::Array{Float64,3}
    W_div_R::Array{Float64,5}
    W_div::Array{Float64,5}
    σ_div_R::Array{Float64,6}
    # σ_div_D::Array{Float64,4}
    σ_div_d::Array{Float64,6}
    σ_div::Array{Float64,6}
...
function variables_function(
    ...
    v_div_R = zeros(a_size, a_size, z_size, η_size, κ_size, lifespan)
    W_div_D = zeros(z_size, η_size, lifespan)
    W_div_R = zeros(a_size, z_size, η_size, κ_size, lifespan)
    W_div = zeros(a_size, z_size, η_size, κ_size, lifespan)

    # Divorced choice probabilities
    σ_div_R = ones(a_size, a_size, z_size, η_size, κ_size, lifespan) ./ (a_size)
    σ_div_d = ones(d_size, a_size, z_size, η_size, κ_size, lifespan) ./ d_size
    σ_div = ones(a_size+1, a_size, z_size, η_size, κ_size, lifespan) ./ (a_size+1)
...
function value_function_singles!(
    ...
    if age != lifespan
        W_expect_mat = reshape(variables.W_S[:,:,:,:,age+1],a_size,:)*transpose(kron(reshape(Γ_κ,1,:),kron(reshape(Γ_η,1,:),Γ_z)))
    end
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        if c <= 0.0
            ...
            if age == lifespan
                ...
                @inbounds variables.W_S_D[z_i, η_i, age] = utility_function(c/es_grid[age]) + β*W_expect_mat[a_ind_zero,z_i]
                ...
                for a_p_i in 1:a_size
                    ...
                    if c <= 0.0
                        ...
                        if age == lifespan
                            ...
                            @inbounds variables.v_S_R[a_p_i, a_i, z_i, η_i, κ_i, age] = utility_function(c/es_grid[age]) + β*W_expect_mat[a_p_i,z_i]
...
function value_function_divorced!(
    ...
    @unpack  a_size, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero, BM_indices, α, ζ, L_ζ, κ_div, lifespan, h_grid, es_grid = parameters
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        ...
        if c <= 0.0
            @inbounds variables.W_div_D[z_i, η_i, age] = -Inf
        else
            if age == lifespan
                @inbounds variables.W_div_D[z_i, η_i, age] = utility_function(c/es_grid[age])
            else
                # W_expect = 0.0
                #
                # for z_p_i in 1:z_size, η_p_i in 1:η_size, κ_p_i in 1:κ_size
                ...
                @inbounds variables.W_div_D[z_i, η_i, age] = utility_function(c/es_grid[age]) + β*W_expect_mat[a_ind_zero,z_i]
                # @inbounds variables.W_div_D[z_i, η_i, age] = utility_function(c/es_grid[age]) + β*W_expect
            end
        end
        ...
        for κ_i in 1:κ_size
            ...
            for a_i in 1:a_size
                ...
                for a_p_i in 1:a_size
                    ...
                    @inbounds c = z*η*h_grid[age]+a-variables.q_S[a_p_i,z_i,age]*a_p - κ - κ_div

                    if c <= 0.0
                        @inbounds variables.v_div_R[a_p_i, a_i, z_i, η_i, κ_i, age] = -Inf
                    else
                        if age == lifespan
                            variables.v_div_R[a_p_i, a_i, z_i, η_i, κ_i, age] = utility_function(c/es_grid[age])
                        else
                            # W_expect = 0.0
                            #
                            # for z_p_i in 1:z_size, η_p_i in 1:η_size, κ_p_i in 1:κ_size
                            ...
                            @inbounds variables.v_div_R[a_p_i, a_i, z_i, η_i, κ_i, age] = utility_function(c/es_grid[age]) + β*W_expect_mat[a_p_i,z_i]
                            # @inbounds variables.v_div_R[a_p_i, a_i, z_i, η_i, κ_i, age] = utility_function(c/es_grid[age]) + β*W_expect
                        end
                    end
                end
            end
        ...
        for a_i in 1:a_size
            ...
            for κ_i in 1:κ_size
                ...
                @inbounds @views v_div_R_max = maximum(variables.v_div_R[:,a_i,z_i,η_i,κ_i,age])
                if v_div_R_max == -Inf
                    @inbounds variables.W_div_R[a_i,z_i,η_i,κ_i,age] = v_div_R_max
                    # variables.σ_S_R[:,:,a_i,z_i,η_i,κ_i] .= 1.0/(a_size*n_size)
                    @inbounds variables.σ_div_R[:,a_i,z_i,η_i,κ_i,age] .= 0.0
                else
                    @inbounds @views v_div_R_diff = variables.v_div_R[:,a_i,z_i,η_i,κ_i,age] .- v_div_R_max
                    v_div_R_exp = exp.(v_div_R_diff ./ ζ)
                    v_div_R_sum = sum(v_div_R_exp)
                    @inbounds variables.W_div_R[a_i,z_i,η_i,κ_i,age] = v_div_R_max + ζ*log(v_div_R_sum)
                    @inbounds variables.σ_div_R[:,a_i,z_i,η_i,κ_i,age] = v_div_R_exp ./ v_div_R_sum
                end

                # compute defaut/repayment probabilities
                @inbounds d_vec = vcat(variables.W_div_D[z_i,η_i,age],variables.W_div_R[a_i,z_i,η_i,κ_i,age])
                d_max = maximum(d_vec)
                d_diff = d_vec .- d_max
                d_exp = exp.(d_diff ./ ζ)
                d_sum = sum(d_exp)
                @inbounds variables.W_div[a_i,z_i,η_i,κ_i,age] = d_max + ζ*log(d_sum)
                @inbounds variables.σ_div_d[:,a_i,z_i,η_i,κ_i,age] = d_exp ./ d_sum

                @inbounds variables.σ_div[1,a_i,z_i,η_i,κ_i,age] = variables.σ_div_d[1,a_i,z_i,η_i,κ_i,age]
                @inbounds variables.σ_div[2:end,a_i,z_i,η_i,κ_i,age] = variables.σ_div_d[2,a_i,z_i,η_i,κ_i,age] .* variables.σ_div_R[:,a_i,z_i,η_i,κ_i,age]
...
function value_function_couples!(
    ...
    @unpack  a_size, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero, λ, BM_indices, α, ζ, L_ζ, ψ, lifespan, h_grid, es_grid = parameters

    if age != lifespan
        W_expect_C_mat = reshape(variables.W_C[:,:,:,:,:,:,:,age+1],a_size,:)*transpose(kron(reshape(parameters.Γ_κ,1,:),kron(reshape(parameters.Γ_κ,1,:),kron(reshape(parameters.Γ_η,1,:),kron(reshape(parameters.Γ_η,1,:),kron(parameters.Γ_z,parameters.Γ_z))))))

        W_expect_div_mat = reshape(variables.W_div[:,:,:,:,age+1],a_size,:)*transpose(kron(reshape(parameters.Γ_κ,1,:),kron(reshape(parameters.Γ_η,1,:),parameters.Γ_z)))
    end
    ...
    Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
        for η_2_i in 1:η_size, z_2_i in 1:z_size
            ...
            c = (z_1*η_1*h_grid[age]+z_2*η_2*h_grid[age])*(1.0-ϕ)

            if c <= 0.0
                @inbounds variables.W_C_D[z_1_i, z_2_i, η_1_i, η_2_i, age] = -Inf
            else
                if age == lifespan
                    variables.W_C_D[z_1_i, z_2_i, η_1_i, η_2_i, age] = λ*utility_function(c/(2*es_grid[age])) + (1-λ)*utility_function(c/(2*es_grid[age]))
                else
                    # W_expect_C = 0.0
                    #
                    # for z_p_1_i in 1:z_size, z_p_2_i in 1:z_size, η_p_1_i in 1:η_size, η_p_2_i in 1:η_size, κ_p_1_i in 1:κ_size, κ_p_2_i in 1:κ_size
                    #     W_expect_C += Γ_z[z_1_i,z_p_1_i]*Γ_z[z_2_i,z_p_2_i]*Γ_η[η_p_1_i]*Γ_η[η_p_2_i]*Γ_κ[κ_p_1_i]*Γ_κ[κ_p_2_i]*variables.W_C[a_ind_zero,z_p_1_i,z_p_2_i,η_p_1_i,η_p_2_i,κ_p_1_i,κ_p_2_i,age+1]
                    # end
                    #
                    # W_expect_div_1 = 0.0
                    #
                    # for z_p_1_i in 1:z_size, η_p_1_i in 1:η_size, κ_p_1_i in 1:κ_size
                    #     W_expect_div_1 += Γ_z[z_1_i,z_p_1_i]*Γ_η[η_p_1_i]*Γ_κ[κ_p_1_i]*variables.W_div[a_ind_zero,z_p_1_i,η_p_1_i,κ_p_1_i,age+1]
                    # end
                    #
                    # W_expect_div_2 = 0.0
                    #
                    # for z_p_2_i in 1:z_size, η_p_2_i in 1:η_size, κ_p_2_i in 1:κ_size
                    #     W_expect_div_2 += Γ_z[z_2_i,z_p_2_i]*Γ_η[η_p_2_i]*Γ_κ[κ_p_2_i]*variables.W_div[a_ind_zero,z_p_2_i,η_p_2_i,κ_p_2_i,age+1]
                    # end

                    # @inbounds variables.W_C_D[z_1_i, z_2_i, η_1_i, η_2_i, age] = λ*utility_function(c/(2*es_grid[age])) + (1-λ)*utility_function(c/(2*es_grid[age]))+ β*(1-ψ)*W_expect_C + β*ψ*(λ*W_expect_div_1 + (1-λ)*W_expect_div_2)

                    @inbounds variables.W_C_D[z_1_i, z_2_i, η_1_i, η_2_i, age] = λ*utility_function(c/(2*es_grid[age])) + (1-λ)*utility_function(c/(2*es_grid[age]))+ β*(1-ψ)*W_expect_C_mat[a_ind_zero,z_i] + β*ψ*(λ*W_expect_div_mat[a_ind_zero, z_1_i] + (1-λ)*W_expect_div_mat[a_ind_zero, z_2_i])
                end
            end

        # Repayment

        for κ_1_i in 1:κ_size, κ_2_i in 1:κ_size, a_i in 1:a_size
            ...
            for a_p_i in 1:a_size
                ...
                @inbounds c = z_1*η_1*h_grid[age]+z_2*η_2*h_grid[age]+a-variables.q_C[a_p_i,z_1_i,z_2_i,age]*a_p-κ_1-κ_2
                ...
                if c <= 0.0
                    ...
                    if age == lifespan
                        @inbounds variables.v_C_R[a_p_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i, κ_1_i, κ_2_i, age] = λ*utility_function(c/(2*es_grid[age])) + (1-λ)*utility_function(c/(2*es_grid[age]))
                    else
                        ...
                        #
                        # for z_p_1_i in 1:z_size, z_p_2_i in 1:z_size, η_p_1_i in 1:η_size, η_p_2_i in 1:η_size, κ_p_1_i in 1:κ_size, κ_p_2_i in 1:κ_size
                        #     W_expect_C += Γ_z[z_1_i,z_p_1_i]*Γ_z[z_2_i,z_p_2_i]*Γ_η[η_p_1_i]*Γ_η[η_p_2_i]*Γ_κ[κ_p_1_i]*Γ_κ[κ_p_2_i]*variables.W_C[a_p_i,z_p_1_i,z_p_2_i,η_p_1_i,η_p_2_i,κ_p_1_i,κ_p_2_i,age+1]
                        # end
                        #
                        # W_expect_div_1 = 0.0
                        #
                        # for z_p_1_i in 1:z_size, η_p_1_i in 1:η_size, κ_p_1_i in 1:κ_size
                        #     W_expect_div_1 += Γ_z[z_1_i,z_p_1_i]*Γ_η[η_p_1_i]*Γ_κ[κ_p_1_i]*variables.W_div[a_p_i_half_1,z_p_1_i,η_p_1_i,κ_p_1_i,age+1]
                        # end
                        #
                        # W_expect_div_2 = 0.0
                        #
                        # for z_p_2_i in 1:z_size, η_p_2_i in 1:η_size, κ_p_2_i in 1:κ_size
                        #     W_expect_div_2 += Γ_z[z_2_i,z_p_2_i]*Γ_η[η_p_2_i]*Γ_κ[κ_p_2_i]*variables.W_div[a_p_i_half_2,z_p_2_i,η_p_2_i,κ_p_2_i,age+1]
                        # end

                        # @inbounds variables.v_C_R[a_p_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i, κ_1_i, κ_2_i, age] = λ*utility_function(c/(2*es_grid[age])) + (1-λ)*utility_function(c/(2*es_grid[age]))+ β*(1-ψ)*W_expect_C + β*ψ*(λ*W_expect_div_1 + (1-λ)*W_expect_div_2)

                        @inbounds variables.v_C_R[a_p_i, a_i, z_1_i, z_2_i, η_1_i, η_2_i, κ_1_i, κ_2_i, age] = λ*utility_function(c/(2*es_grid[age])) + (1-λ)*utility_function(c/(2*es_grid[age]))+ β*(1-ψ)*W_expect_C_mat[a_p_i,z_i] + β*ψ*(λ*W_expect_div_mat[a_p_i_half_1, z_1_i] + (1-λ)*W_expect_div_mat[a_p_i_half_2, z_2_i])
                    end
                end
            end
        end
    ...
    if age == lifespan
        ...
        Threads.@threads for a_p_i in 1:(a_ind_zero-1)
            ...
            for z_2_i in 1:z_size, z_1_i in 1:z_size
                ...
                for η_2_p_i in 1:η_size, η_1_p_i in 1:η_size, z_2_p_i in 1:z_size, z_1_p_i in 1:z_size, κ_1_p_i in 1:κ_size, κ_2_p_i in 1:κ_size

                    recovery = ((z_grid[z_1_p_i]*η_grid[η_1_p_i]*h_grid[age+1]+z_grid[z_2_p_i]*η_grid[η_2_p_i]*h_grid[age+1])*parameters.ϕ)/(abs(a_grid[a_p_i])+κ_grid[κ_1_p_i]+κ_grid[κ_2_p_i])

                    if recovery > 1.0
                        recovery = 1.0
                    end

                    @inbounds P_expect_remain += Γ_z[z_1_i,z_1_p_i]*Γ_z[z_2_i,z_2_p_i]*Γ_η[η_1_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_1_p_i]*Γ_κ[κ_2_p_i]*((1.0-variables.σ_C[1,a_p_i,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i,κ_1_p_i,κ_2_p_i,age+1])+(variables.σ_C[1,a_p_i,z_1_p_i,z_2_p_i,η_1_p_i,η_2_p_i,κ_1_p_i,κ_2_p_i,age+1]*recovery))
                end

                for κ_1_p_i in 1:κ_size, η_1_p_i in 1:η_size, z_1_p_i in 1:z_size
                    recovery = ((z_grid[z_1_p_i]*η_grid[η_1_p_i]*h_grid[age+1])*parameters.ϕ)/(abs(a_grid[a_p_i_half_1])+κ_grid[κ_1_p_i])

                    if recovery > 1.0
                        recovery = 1.0
                    end

                    @inbounds P_expect_div_1 += Γ_z[z_1_i,z_1_p_i]*Γ_η[η_1_p_i]*Γ_κ[κ_1_p_i]*((1.0-variables.σ_div[1,a_p_i_half_1,z_1_p_i,η_1_p_i,κ_1_p_i,age+1])+(variables.σ_div[1,a_p_i_half_1,z_1_p_i,η_1_p_i,κ_1_p_i,age+1]*recovery))
                end

                for κ_2_p_i in 1:κ_size, η_2_p_i in 1:η_size, z_2_p_i in 1:z_size
                    recovery = ((z_grid[z_2_p_i]*η_grid[η_2_p_i]*h_grid[age+1])*parameters.ϕ)/(abs(a_grid[a_p_i_half_2])+κ_grid[κ_2_p_i])

                    if recovery > 1.0
                        recovery = 1.0
                    end

                    @inbounds P_expect_div_2 += Γ_z[z_2_i,z_2_p_i]*Γ_η[η_2_p_i]*Γ_κ[κ_2_p_i]*((1.0-variables.σ_div[1,a_p_i_half_2,z_2_p_i,η_2_p_i,κ_2_p_i,age+1])+(variables.σ_div[1,a_p_i_half_2,z_2_p_i,η_2_p_i,κ_2_p_i,age+1]*recovery))
                end
