...
function value_function_singles!(
    ...
    @unpack  a_size, z_size, η_size, κ_size, a_grid, z_grid, η_grid, κ_grid, T, θ, β, ρ, Γ_z, Γ_η, Γ_κ, ϕ, a_ind_zero, BM_indices, α, n_grid, n_size = parameters

    # V_expect_mat = reshape(V_S_p,a_size,:)*transpose(kron(parameters.Γ_z,kron(reshape(parameters.Γ_η,1,:),reshape(parameters.Γ_κ,1,:))))

    V_expect_mat = reshape(V_S_p,a_size,:)*transpose(kron(reshape(parameters.Γ_κ,1,:),kron(reshape(parameters.Γ_η,1,:),parameters.Γ_z)))
            ...
            if ub != 0
                ...
                for a_p_i in lb:ub, n_i in 1:n_size
                    ...
                    if optf > variables.V_S[a_i,z_i,η_i,κ_i]
                        ...
                        @inbounds variables.n_S_i[a_i,z_i,η_i,κ_i] = n
...
using JLD2
@save "workspace.jld2" parameters V_S V_C n_S_i n_C_1_i n_C_2_i a_S_i a_C_i d_S_i d_C_i P_S q_S P_C q_C μ_S μ_C
