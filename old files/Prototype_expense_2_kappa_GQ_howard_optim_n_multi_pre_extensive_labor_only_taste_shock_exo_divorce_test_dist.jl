        ...
        Threads.@threads for i in CartesianIndices((1:z_size,1:η_size))
            ...
            for κ_p_i in 1:κ_size
                ...
                for κ_i in 1:κ_size, η_i in 1:η_size, z_i in 1:z_size, a_i in 1:a_size
                    ...
                    @inbounds @views μ_temp_survivor .+= dropdims(sum(variables.σ_S[2:end,:,a_i,z_i,η_i,κ_i],dims=2),dims=2)*(ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*μ_S_p[a_i,z_i,η_i,κ_i])

                    @inbounds @views μ_temp_survivor[a_ind_zero] += ρ*Γ_z[z_i,z_p_i]*Γ_η[η_p_i]*Γ_κ[κ_p_i]*sum(variables.σ_S[1,:,a_i,z_i,η_i,κ_i])*μ_S_p[a_i,z_i,η_i,κ_i] + (1-ρ)*Γ_η[η_p_i]*(z_p_i == 3)*(κ_p_i == 1)*μ_S_p[a_i,z_i,η_i,κ_i]
                end
